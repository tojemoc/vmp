...
- On confirmation — **durable, retry-safe deletion job** (do **not** rely on a bare `DELETE FROM users` or a single fire-and-forget handler):
  ...
  4. **Explicit cleanup of non-CASCADE FK tables** (same `db.batch()` as step 3):
     ...
     - Deletion-token rows: omit if the table uses `ON DELETE CASCADE`; otherwise `DELETE FROM <deletion_tokens> WHERE user_id = ?` in this same batch (or rely on `ON DELETE SET NULL` plus post-job purge)
  ...
  6. **Brevo contact deletion** (mandatory when `BREVO_API_KEY` is configured): capture the contact identifier (email) **before** local user data is removed; call `DELETE /contacts/{identifier}` via a dedicated deletion worker/adapter (not `removeSubscriberFromNewsletter`, which only removes list membership). Treat **2xx and 404** as successful completion and persist `brevo_deleted`; retry only transient failures (5xx, timeouts). Stale-contact sync is a backstop only, never the primary deletion path.
...
- New migration(s): `account_deletion_jobs` + `account_deletion_r2_objects` + deletion token table (separate from `magic_link_tokens`, with `user_id` `ON DELETE CASCADE` or `ON DELETE SET NULL`) **and** `einvoices` FK/retention fix described above.
