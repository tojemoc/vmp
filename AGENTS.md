...
**Staging SideStore auth escape hatch (temporary):** Mobile artifacts with `flavor=development` + `enable_custom_scheme=true` may enable claimable `vmp://`. Staging web (`deployTier=staging`) plus API `ALLOW_INSECURE_NATIVE_VMP_SCHEME=1` **and** `SENTRY_ENVIRONMENT=staging` (both set by staging CD `--var`) requires a two-step confirm and records `insecure_native_scheme_acks` in D1 before opening the scheme. Check `GET /api/auth/native/insecure-scheme/status` → `allowed: true`. Production / beta never enable this. Prefer Universal Links (**S5**) when a stable Team ID exists. Do **not** paste Cloudflare “sync wrangler.jsonc” dumps that turn `--var KEY=value` into a var *named* `KEY=value` with an empty string.
