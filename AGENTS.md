...
- **Deno Deploy backup API (`@vmp/api-node`)**: PR CI job `api-node` runs `npm run verify:api-node` (typecheck, tests, build). Deploy job `deploy-api-node-backup` repeats verify before upload and smoke-checks `/api/health` + `/api/homepage/content` after upload. **Do not merge PRs that fail api-node CI** — a green upload alone does not prove the backup API serves traffic correctly.
