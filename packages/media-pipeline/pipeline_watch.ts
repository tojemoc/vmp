...
type PipelineStage =
  ...
  | 'phase2_package'
  | 'phase2_upload'
  | 'phase2_manifest_swap'
  | 'multi_rendition_ready'
  | 'podcast_mp3'
  | 'preview_wait'
  | 'preview_render'
  | 'preview_upload'
...
type ResolveInputTargetPathResult = {
  videoId: string;
  targetPath: string;
  renamed: boolean;
...
const INBOX_FAST_LANE_DIR =
  (process.env.INBOX_FAST_LANE_DIR || '').trim() ||
  (LEGACY_INBOX_DIR ? path.join(LEGACY_INBOX_DIR, 'fast-lane') : '/mnt/videos/inbox-fast-lane');
const INBOX_FULL_LADDER_DIR =
  (process.env.INBOX_FULL_LADDER_DIR || '').trim() ||
...
const WAIT_STABLE_TIMEOUT_MS = Math.max(
  1_000,
  Number.parseInt(process.env.WAIT_STABLE_TIMEOUT_MS || '120000', 10) || 120000,
);
const WAIT_STABLE_POLL_MS = Math.max(
  250,
  Number.parseInt(process.env.WAIT_STABLE_POLL_MS || '2000', 10) || 2000,
);
const WAIT_STABLE_IDLE_POLLS = Math.max(
  1,
  Number.parseInt(process.env.WAIT_STABLE_IDLE_POLLS || '1', 10) || 1,
);
...
const PIPELINE_CALLBACK_TIMEOUT_MS = Math.max(
  5_000,
...
async function cleanupJobArtifacts(
  videoId: string,
  lockFile?: string,
  inputPath?: string,
): Promise<void> {
...
async function cleanupCancelledJob(
  videoId: string,
  lockFile: string,
  inputPath?: string,
): Promise<void> {
...
function stageToProgressPhase(stage: PipelineStage): ProgressPhase {
  ...
  if (stage === 'probe' || stage === 'wait_upload_complete' || stage === 'detected')
    return 'phase1';
  return 'phase1';
}
...
function run(
  ...
  args: string[],
  label: string,
  { capture = false, videoId }: RunOptions = {},
): Promise<RunResult> {
  return new Promise<RunResult>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      reject(
        new Error(`${label} failed exit=${code}${capture ? ` stderr=${stderr.slice(-400)}` : ''}`),
      );
    });
  });
}
...
function sanitizeVideoId(stem: string): string {
  ...
  if (VIDEO_ID_SANITIZE_MODE === 'base64url')
    return Buffer.from(stem).toString('base64url') || 'dmlkZW8';
  const slug =
    stem
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
...
async function probeSource(
  filePath: string,
  videoId?: string,
): Promise<{ hasAudio: boolean; durationSec: number | null }> {
  let hasAudio = false;
  try {
...
async function notifyVideoAvailable(
  ...
  if (!VMP_API_BASE_URL || !VMP_API_PIPELINE_SECRET) {
    log(
      `⚠️ ${videoId}: pipeline status callback skipped (VMP_API_BASE_URL or VMP_API_PIPELINE_SECRET unset)`,
    );
    return;
  }
  ...
  const attempt = async (retry: boolean): Promise<void> => {
    ...
    const signature = crypto
      .createHmac('sha256', VMP_API_PIPELINE_SECRET)
...
async function completePipelineSuccess(
  ...
  histogram('vmp.transcoder.job.duration_ms', Date.now() - jobStartMs, {
    outcome: 'success',
    source,
  });
...
async function reclaimOrSkipLock(lockFile: string, videoId: string): Promise<'skip' | 'proceed'> {
  ...
  log(
    `♻️  ${videoId} reclaiming stale lock (owner=${Number.isFinite(ownerPid) ? ownerPid : 'unknown'})`,
  );
...
async function processVideo(
  ...
  try {
    ...
    emitPipelineEvent(
      ...
      'probe',
      'active',
      `hasAudio=${hasAudio}${durationSec != null ? ` durationSec=${durationSec.toFixed(1)}` : ''}`,
    );
    ...
    await runQueuedPipelineJob({
      ...
      notifyVideoAvailable: (stage, renditions) =>
        notifyVideoAvailable(videoId, stage, renditions as RenditionKey[]),
    });
...
function scheduleInboxIntake(
  ...
  inboxIntakeChain = inboxIntakeChain
    ...
    .catch((err) => {
      log(
        `inbox intake failed for '${file}' (${inboxDir}): ${err instanceof Error ? err.message : String(err)}`,
      );
    });
}

async function intakeInboxBasename(
  file: string,
  inboxDir: string,
  pipelineMode: PipelineMode,
  mechanism: string,
  ...
  if (result.renamed) {
    const detail =
      result.reason === 'random_uuid' ? 'Generated random VIDEO_ID' : 'Sanitized VIDEO_ID';
...
async function startIpcServer(): Promise<void> {
  ...
  ipcServer = net.createServer((socket) => {
    ...
    socket.on('error', () => {
      try {
        socket.end(JSON.stringify({ ok: false, error: 'socket_error' }));
      } catch {}
    });
  });
...
async function resolveInputTargetPath(
  stem: string,
  ext: string,
  oldPath: string,
  source: string,
): Promise<ResolveInputTargetPathResult> {
  ...
  if (VIDEO_ID_STRATEGY === 'filename') {
    ...
    while (attempt < 1000) {
      ...
      if (!existsSync(candidatePath)) {
        ...
        if (attempt > 0) {
          log(
            `⚠️ Filename collision in ${source}: '${oldPath}' -> '${candidatePath}' (base '${baseId}' already existed)`,
          );
        }
        return {
          videoId: candidateId,
...
function startWatcherForInbox(watch: InboxWatchConfig) {
  const child = trackChild(
    spawn('inotifywait', ['-m', '-e', 'close_write', '--format', '%f', watch.dir], {
      env: process.env,
...
async function main() {
  ...
  try {
    ...
    log(
      `✅ inotify watcher started (${watchers.length} inbox${watchers.length === 1 ? '' : 'es'})`,
    );
  } catch (err) {
    log(
      `⚠️ inotify unavailable, falling back to polling: ${err instanceof Error ? err.message : String(err)}`,
    );
