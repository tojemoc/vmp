...
function parseArgs(): { videoId: string; previewSeconds: number } {
  ...
  if (!rawVideoId)
    throw new Error('Usage: node dist/render_podcast_preview_mp3.js <video_id> <preview_seconds>');
  if (!/^[a-zA-Z0-9._-]+$/.test(rawVideoId))
    throw new Error('video_id contains invalid characters');
...
async function main() {
  ...
  try {
    ...
    if (
      ...
      console.log(
        `[preview] skip full-unlock video=${videoId} (${previewSeconds}s >= ${Math.round(sourceDurationSeconds)}s source)`,
      );
      return;
    }
    ...
    await uploadFileToStorage(
      localOut,
      objectKey('videos', videoId, 'podcast_preview.mp3'),
      'preview upload',
    );
    await uploadFileToStorage(
      localMeta,
      objectKey('videos', videoId, 'podcast_preview.meta.json'),
      'preview metadata upload',
    );
