  ...
  return {
    prepare(sql: string) {
      ...
      const stmt = {
        ...
        async first() {
          if (query.includes('sum(case when provider')) {
            ...
            return {
              ...
              not_validated: legacy.filter(
                (r) => r.status === 'needs_relink' && r.legacy_validation_status == null,
              ).length,
            };
          }
        ...
        async all() {
          if (
            query.includes('legacy_validation_status is null') &&
            query.includes('needs_relink')
          ) {
          ...
          if (query.includes('join users')) {
            ...
            const matched = rows
              .filter((r) => {
                if (r.legacy_validation_status === 'invalid' && r.provider === 'legacy')
                  return true;
                if (r.status === 'needs_relink') {
                  if (!cutoff) return true;
                  if (r.created_at) return new Date(String(r.created_at)).getTime() <= cutoff;
