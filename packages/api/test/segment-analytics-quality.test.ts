...
describe('segment analytics source classification', () => {
  ...
  it('classifies search/social/referral/direct', () => {
    assert.equal(
      classifySegmentSource({ referer: 'https://www.google.com/search?q=vmp' }).category,
      'search',
    );
    assert.equal(classifySegmentSource({ referer: 'https://t.co/xyz' }).category, 'social');
    assert.equal(
      classifySegmentSource({ referer: 'https://partner.example.net/article' }).category,
      'referral',
    );
...
describe('segment playback + session derivation', () => {
  it('derives playback position from segment index and duration', () => {
    const playback = derivePlaybackPositionSeconds({
      segmentIndex: 5,
      segmentDurationSeconds: 6.2,
    });
  ...
  it('builds stable session keys in 30-minute buckets', () => {
    const early = buildSegmentSessionKey({
      videoId: 'vid-1',
      userId: 'u-1',
      timestampMs: 1_700_000_000_000,
    });
    const laterSameBucket = buildSegmentSessionKey({
      videoId: 'vid-1',
      userId: 'u-1',
      timestampMs: 1_700_000_100_000,
    });
    const nextBucket = buildSegmentSessionKey({
      videoId: 'vid-1',
      userId: 'u-1',
...
describe('segment analytics persistence + snapshot', () => {
  function buildDbStub(
    resultsByMatch: Array<{ match: string; response: any }>,
      ...
      prepare(sql: string) {
        if (sql.includes('admin_settings')) {
          return {
            ...
            async all() {
              ...
              const results = keys
                .filter((key: string) => Object.hasOwn(settings, key))
        ...
        return {
          ...
          async first() {
            ...
            if (matched.response && Object.hasOwn(matched.response, 'first')) {
          ...
          async all() {
            ...
            if (matched.response && Object.hasOwn(matched.response, 'all')) {
  ...
  it('writes canonical source/session/position fields on event log', async () => {
    ...
    const insert = captured.find((entry) =>
      String(entry.sql).includes('INSERT INTO video_segment_events'),
    );
    ...
    const db = buildDbStub([
      { match: 'COUNT(DISTINCT session_id) AS total', response: { first: { total: 4 } } },
      {
        ...
        response: {
          all: {
            results: [
              { source: 'search', unique_sessions: 3 },
              { source: 'direct', unique_sessions: 1 },
            ],
          },
        },
      },
      {
        match: 'SELECT AVG(session_retention_pct) AS average_retention_percent',
        response: { first: { average_retention_percent: 55.5 } },
      },
      {
        match: 'per_video AS',
        response: {
            ...
            results: [
              {
                ...
                full_duration: 100,
                view_count: 2,
                average_retention_percent: 55.5,
                completion_rate_percent: 20,
                total_watch_seconds: 300,
              },
            ],
          },
        },
      ...
      {
        match: 'SELECT COALESCE(status, ',
        response: { all: { results: [{ status: 'active', count: 5 }] } },
      },
      {
      ...
      },
      {
        ...
        response: { all: { results: [{ bucket: '2026-04-01', total_watch_seconds: 600 }] } },
      },
      {
        match:
        ...
        response: { first: { total_watch_seconds: 1200 } },
      },
      {
        match: 'SELECT COUNT(*) AS total\n      FROM video_segment_events',
        response: { first: { total: 42 } },
      },
      {
        match:
      ...
      },
      {
        match:
          'SELECT\n        country_code AS country,\n        SUM(COALESCE(segment_duration_seconds, 6)) AS total_watch_seconds',
        response: { all: { results: [{ country: 'SK', total_watch_seconds: 900 }] } },
      },
    ...
    const env = {
      DB: buildDbStub(
        [
          ...
          {
            match: 'SELECT AVG(session_retention_pct) AS average_retention_percent',
            response: { first: { average_retention_percent: 42 } },
          },
          {
            match: 'per_video AS',
            response: {
                ...
                results: [
                  {
                    ...
                    full_duration: 100,
                    view_count: 4,
                    average_retention_percent: 42,
                    completion_rate_percent: 10,
                    total_watch_seconds: 500,
                  },
                ],
              },
            },
          ...
          {
            match: 'SELECT COALESCE(status, ',
            response: { all: { results: [{ status: 'active', count: 8 }] } },
          },
          {
            match: 'SELECT\n        bucket,\n        COUNT(DISTINCT session_id) AS unique_sessions',
            response: { all: { results: [{ bucket: '2026-04-01', unique_sessions: 3 }] } },
          },
          {
            match:
              'AS bucket,\n        SUM(COALESCE(segment_duration_seconds, 6)) AS total_watch_seconds',
            response: { all: { results: [{ bucket: '2026-04-01', total_watch_seconds: 800 }] } },
          },
          {
            match:
              "SELECT SUM(COALESCE(segment_duration_seconds, 6)) AS total_watch_seconds\n      FROM video_segment_events\n      WHERE event_type = 'segment'\n        AND datetime(created_at) >= datetime(?)",
            response: { first: { total_watch_seconds: 2400 } },
          },
          {
            match: 'SELECT COUNT(*) AS total\n      FROM video_segment_events',
            response: { first: { total: 99 } },
          },
          {
            match:
              'SELECT\n        country_code AS country,\n        COUNT(DISTINCT session_id) AS unique_sessions',
            response: { all: { results: [{ country: 'CZ', unique_sessions: 1 }] } },
          },
          {
            match:
              'SELECT\n        country_code AS country,\n        SUM(COALESCE(segment_duration_seconds, 6)) AS total_watch_seconds',
            response: { all: { results: [{ country: 'CZ', total_watch_seconds: 700 }] } },
          },
          {
            match: 'AS new_subscriptions',
            response: { all: { results: [{ bucket: '2026-04-01', new_subscriptions: 2 }] } },
          },
          {
            match: 'AS churned_subscriptions',
            response: { all: { results: [{ bucket: '2026-04-01', churned_subscriptions: 1 }] } },
          },
          {
            match: 'AS expiring_subscriptions',
            response: { all: { results: [{ bucket: '2026-04-01', expiring_subscriptions: 1 }] } },
          },
          {
            match: 'SELECT plan_type, COUNT(*) AS active_count',
