...
import {
  ...
  sortCategoriesForHomepage,
} from '../src/homepagePlacement.js';
...
describe('placeHomepageVideos matrix', () => {
  it('1 — reassign A → B: video appears only under B (not A)', () => {
    ...
    assert.ok(
      before.categoryBlocks
        .find((b) => b.category.id === 'c-a')
        ?.visible.some((v) => v.id === 'v-move'),
    );
    assert.equal(
      before.categoryBlocks.find((b) => b.category.id === 'c-b')?.visible.length ?? 0,
      0,
    );
  ...
  it('6 — four in one category: 3 visible, overflow 1; direction respected (asc)', () => {
    ...
    assert.deepEqual(
      block.visible.map((v) => v.id),
      ['v2', 'v3', 'v4'],
    );
    assert.deepEqual(
      block.overflow.map((v) => v.id),
      [],
    );
  });
  ...
  it('featuredVideoIds pins up to four videos and removes them from categories', () => {
    ...
    assert.deepEqual(
      out.featured.map((v) => v.id),
      ['p1', 'p2'],
    );
    assert.deepEqual(
      out.categoryBlocks[0]?.visible.map((v) => v.id),
      ['p3'],
    );
  });
  ...
  it('layoutIncludesFeaturedRowBlock detects split children', () => {
    assert.equal(
      layoutIncludesFeaturedRowBlock([
        { type: 'split_horizontal', childBlocks: [{ type: 'featured_row' }] },
      ]),
      true,
    );
