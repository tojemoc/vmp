...
describe('offlineManifest helpers', () => {
  ...
  it('parses licensed manifest path sets', () => {
    const paths = parseLicensedManifestPaths(
      JSON.stringify(['720p/seg_720_001.m4s', 'audio.m3u8']),
    );
  ...
  it('builds a manifest from an in-memory R2 reader', async () => {
    const reader = createFixtureReader({
      ...
      'audio/playlist.m3u8': ['#EXTM3U', '#EXTINF:4.0,', 'seg_001.m4s'].join('\n'),
      '720p/playlist.m3u8': ['#EXTM3U', '#EXTINF:4.0,', 'seg_001.m4s'].join('\n'),
