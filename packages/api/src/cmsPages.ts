...
export async function handleCmsPageBySlug(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  slug: string,
  ...
  if (!page || isCmsSystemSlug(slug))
    return jsonResponse({ error: 'Page not found' }, 404, corsHeaders);
  return jsonResponse({ page }, 200, corsHeaders);
}

export async function handleCmsPageById(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  id: string,
  ...
  if (request.method === 'DELETE') {
    if (isCmsSystemPageId(id)) {
      return jsonResponse(
        { error: 'System pages cannot be deleted', code: 'SYSTEM_PAGE' },
        403,
        corsHeaders,
      );
    }
...
export async function handleCmsPagePublish(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  id: string,
...
export async function handleCmsPageUnpublish(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  id: string,
...
export async function handleCmsPageRevisions(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  id: string,
...
export async function handleCmsMediaUpload(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
) {
...
export async function handleCmsMediaById(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  id: string,
...
export async function handleCmsMediaBatch(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
) {
