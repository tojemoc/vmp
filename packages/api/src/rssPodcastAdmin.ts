...
export async function deliverPodcastPreviewRebuildWebhook(env: any, videos: any[]) {
  const freePreviewEnabled =
    String((await getSetting(env, 'rss_free_preview_enabled', { defaultValue: '1' })) ?? '1') ===
    '1';
  ...
  const webhookUrl = (
    await getSetting(env, 'podcast_rebuild_webhook_url', { defaultValue: '' })
  )?.trim();
  const secret = (
    await getSetting(env, 'podcast_rebuild_webhook_secret', { defaultValue: '' })
  )?.trim();
  if (!webhookUrl || !secret) {
    return {
      ...
      eligibleCount: 0,
      skippedFullUnlockCount: videos.filter(
        (v) => v?.id && !needsPodcastPreviewMp3(v.preview_duration, v.full_duration),
      ).length,
    };
  }
...
export async function handleRssPodcastWebhookConfig(request: any, env: any, corsHeaders: any) {
  ...
  try {
    ...
    const webhookUrl: string | null = rawUrl || null;
    if (webhookUrl) {
      try {
    ...
    if (secretRaw != null) {
      ...
      if (trimmed.length === 0) {
        ...
        return jsonResponse(
          { error: 'webhookSecret must be at least 16 characters when set' },
          400,
          corsHeaders,
        );
      } else {
    ...
    const configured =
      (await getSetting(env, 'podcast_rebuild_webhook_secret_set', { defaultValue: null })) === '1';
    return jsonResponse(
      {
        ok: true,
...
export async function handleRssPodcastPreviewRebuildNotify(
  request: any,
  env: any,
  corsHeaders: any,
) {
  ...
  try {
    ...
    if (!freePreviewEnabled) {
      return jsonResponse(
        { error: 'Free podcast preview feed is disabled', code: 'free_preview_disabled' },
        400,
        corsHeaders,
      );
    }
    ...
    if (!result.delivered) {
      if (result.code === 'webhook_not_configured') {
        return jsonResponse(
          { error: 'Podcast rebuild webhook URL is not configured', code: result.code },
          400,
          corsHeaders,
        );
      }
      if (result.code === 'secret_not_configured') {
        return jsonResponse(
          { error: 'Podcast rebuild webhook secret is not configured', code: result.code },
          400,
          corsHeaders,
        );
      }
