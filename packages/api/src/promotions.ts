...
async function validatePromoForPlan(env: any, promoCode: any, planType: string) {
  if (!promoCode)
    return { ok: false, error: 'Promo code not found', code: 'promo_not_found', status: 404 };
  ...
  const allowedPlans = parseAllowedPlanTypes(
    promoCode.allowed_plan_types,
    allowedPlansFromSettings,
  );
  if (!allowedPlans.includes(planType)) {
    return {
      ok: false,
      error: 'Promo code does not apply to this plan',
...
export async function resolvePromoCodeForCheckout(
  env: any,
  codeInput: any,
  planType: string,
...
export async function handleAdminPromoCodes(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'POST') {
    ...
    const campaign = await db
      .prepare('SELECT id FROM promo_campaigns WHERE id = ? LIMIT 1')
      .bind(campaignId)
      .first();
    if (!campaign)
      return jsonResponse(
    ...
    if (expiresAtParsed.invalid)
      return jsonResponse({ error: 'expiresAt must be a valid datetime' }, 400, corsHeaders);
    const expiresAt = expiresAtParsed.value;
    const isActive = body?.isActive === false ? 0 : 1;
    const stripeCouponId =
      typeof body?.stripeCouponId === 'string' ? body.stripeCouponId.trim() : '';
    if (rewardType === 'discount_percent' && !stripeCouponId) {
      return jsonResponse(
        { error: 'Stripe coupon ID is required for discount_percent promo codes' },
        400,
        corsHeaders,
      );
    }
    ...
    for (let i = 0; i < quantity; i += 1) {
      const candidate =
        quantity === 1 && baseCode ? baseCode : `${baseCode || randomCode(6)}-${randomCode(4)}`;
    ...
    return jsonResponse(
      { ok: true, created: createdCodes.length, codes: createdCodes },
      201,
      corsHeaders,
    );
  }
  ...
  if (request.method === 'PATCH') {
    ...
    const existing = await db
      .prepare('SELECT reward_type, stripe_coupon_id FROM promo_codes WHERE id = ? LIMIT 1')
      .bind(id)
      .first();
...
export async function handleAdminIsicCampaigns(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'GET') {
    const [campaignRows, settingsRows] = await Promise.all([
      ...
      `)
        .all(),
      Promise.all([getSetting(env, 'isic_api_enabled'), getSetting(env, 'isic_api_base_url')]),
    ]);
  ...
  if (request.method === 'PATCH') {
    ...
    if (body?.apiConfig && typeof body.apiConfig === 'object') {
      ...
      if (Object.hasOwn(body.apiConfig, 'enabled')) {
        updates.push(['isic_api_enabled', body.apiConfig.enabled ? '1' : '0']);
      }
      if (Object.hasOwn(body.apiConfig, 'baseUrl')) {
        updates.push(['isic_api_base_url', String(body.apiConfig.baseUrl ?? '').trim()]);
      }
      if (Object.hasOwn(body.apiConfig, 'apiKey')) {
        updates.push(['isic_api_key', String(body.apiConfig.apiKey ?? '').trim()]);
      }
      if (!updates.length)
        return jsonResponse({ error: 'No API config fields to update' }, 400, corsHeaders);
    ...
    if (Object.hasOwn(body ?? {}, 'countryScope')) {
      updates.push('country_scope = ?');
      values.push(
        String(body.countryScope ?? 'CZ,SK')
          .trim()
          .toUpperCase() || 'CZ,SK',
      );
    }
...
export async function handleIsicValidate(request: any, env: any, corsHeaders: any) {
  ...
  try {
    ...
    return jsonResponse(
      {
        valid: false,
        error: 'ISIC validation service unavailable',
        code: 'isic_validation_failed',
      },
      500,
      corsHeaders,
