...
async function postReplicationEventsToTarget(
  env: any,
  events: unknown[],
  ...
  for (let attempt = 1; attempt <= REPLICATION_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (isTimeout || isAbort || isNetworkTypeError) {
        ...
        lastError = new Error(
          `Replication target request ${reason} (attempt ${attempt}/${REPLICATION_MAX_ATTEMPTS})`,
        );
...
async function enqueueStreamUsers(
  ...
  const rows = await db
    ...
    LIMIT ?
  `)
    .bind(cursor, cursorParts.updatedAt, cursorParts.updatedAt, cursorParts.id, CURSOR_ROW_LIMIT)
    .all();
  const sorted = rows?.results ?? [];
  if (!sorted.length) return { enqueued: 0, ingest: null };
  const selected = sorted.slice(0, context.batchSize);
  const messages = selected.map((row: any) =>
    buildMessage({
      epoch: context.epoch,
...
async function runReplicationPushRound(env: any, mode: DeliveryMode) {
  ...
  const direction = parseDirection(
    (await getAdminSetting(db, 'replication_mode')) ?? DEFAULT_DIRECTION,
  );
...
export async function pushReplicationToPostgres(env: any) {
  ...
  if (!targetRaw) {
    return {
      ok: false,
      error: 'REPLICATION_TARGET_URL is not configured',
  ...
  if (!token) {
    return {
      ok: false,
      error: 'REPLICATION_TARGET_TOKEN is not configured',
  ...
  for (let roundIndex = 0; roundIndex < maxRounds; roundIndex += 1) {
    ...
    const roundTotal =
      round.counts.users +
      round.counts.subscriptions +
      round.counts.videos +
      round.counts.adminSettings;
...
export async function handleAdminReplicationPush(
  request: Request,
  env: any,
...
export async function handleAdminReplicationSettings(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  if ((body as any).resetCursors === true) {
    ...
    const streams = (body as any).resetStreams
      .map((value: unknown) => String(value))
      .filter((value: string) => STREAMS.includes(value as any));
    if (streams.length > 0) {
