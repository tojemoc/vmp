...
function normalizeReturnPath(raw: unknown, fallback = '/account'): string {
  ...
  if (!trimmed.startsWith('/') || trimmed.startsWith('//') || trimmed.length > 1024)
    return fallback;
  return trimmed;
}
...
export async function startLegacyCheckout(
  ...
  try {
    ...
    `)
      .bind(crypto.randomUUID(), user.sub, planType, idOrder, idOrder)
      .run();
...
export async function handleLegacyCheckout(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
...
export async function handleLegacyComplete(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  try {
    ...
    const planType = normalizePlanType(
      order.planType ?? body?.planType ?? (session as any).plan_type,
    );
...
export async function handleLegacyOrderStatus(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  try {
    const order = (await getLegacyOrder(env, orderId)) as Record<string, unknown>;
    const normalizedStatus = normalizeLegacySubscriptionStatus(
      order.status ?? order.subscriptionStatus,
    );
    return jsonResponse(
      {
        orderId,
...
export async function handleLegacyWebhook(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
  ...
  const sub = await db
    ...
    LIMIT 1
  `)
    .bind(purchaseId, purchaseId, purchaseId)
    .first();

  if (!sub) {
    return jsonResponse(
      { ok: true, skipped: true, reason: 'subscription_not_found' },
      200,
      corsHeaders,
    );
  }
...
export async function handleAdminLegacyPaymentSettings(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
