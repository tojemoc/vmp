...
function buildAdminAuditLogStatement(
  db: any,
  { actorUserId, actionType, targetUserId, detail }: any,
) {
  return db
    .prepare(`
  ...
  `)
    .bind(crypto.randomUUID(), actorUserId, actionType, targetUserId, JSON.stringify(detail ?? {}));
}
...
export async function handleHomepageContent(request: any, env: any, corsHeaders: any) {
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      const homepageConfig = normalizeHomepageConfigForResponse(
        safeJsonParse(homepageRow?.value, null),
      );
      return jsonResponse(
        {
          homepageConfig,
          ...
          precedence: {
            summary:
              'featured → uncategorized recent grid → category sections (P0 categories before standard categories).',
            categoryOrderRule:
              'Categories with sort_order <= 0 are treated as P0 and rendered before all standard categories.',
            overflowRule:
              'Categories with overflow remain in section order and expose remaining videos in overflow metadata.',
          },
        },
        200,
        corsHeaders,
    ...
    if (!configUpdate && !categoryOrderUpdates.length) {
      return jsonResponse(
        { error: 'Provide homepageConfig and/or categoryOrder updates.' },
        400,
        corsHeaders,
      );
    }
    ...
    if (categoryOrderUpdates.length) {
      ...
      const categoryStatements = categoryOrderUpdates.map((entry: any) =>
        updateStmt.bind(entry.sortOrder, entry.id),
      );
  ...
  const layoutBlocks = Array.isArray(input.layoutBlocks)
    ? input.layoutBlocks
        ...
        .map((block: any) => {
          ...
          if (Number.isFinite(Number(block.mobileOrder)))
            normalized.mobileOrder = Number(block.mobileOrder);
          ...
          if (type === 'page_banner') {
            ...
            normalized.mobileImageId =
              typeof block.mobileImageId === 'string' ? block.mobileImageId : '';
...
export async function handlePillsUpdate(request: any, env: any, corsHeaders: any) {
  ...
  if (preAuthRateLimit.error) {
    return jsonResponse(
      { error: preAuthRateLimit.error, code: 'invalid_config' },
      500,
      corsHeaders,
    );
  }
  ...
  const keyValid = expectedHash ? await verifyPillsApiKeyValue(provided, expectedHash) : false;
  if (!keyValid) {
  ...
  const postAuthRateLimit = await checkPillsUpdateRateLimit(request, env, {
    phase: 'key',
    keyFingerprint,
  });
  if (postAuthRateLimit.error) {
    return jsonResponse(
      { error: postAuthRateLimit.error, code: 'invalid_config' },
      500,
      corsHeaders,
    );
  }
  ...
  if (!Array.isArray(body?.pills))
    return jsonResponse({ error: 'pills[] is required' }, 400, corsHeaders);
  ...
  await db
    ...
    VALUES (?, ?, ?, CURRENT_TIMESTAMP)
  `)
    .bind(crypto.randomUUID(), request.headers.get('x-forwarded-for') || 'external-api', nowPayload)
    .run();
  const statements = [];
...
async function checkPillsUpdateRateLimit(
  request: any,
  env: any,
  options: PillsRateLimitOptions = {},
) {
  ...
  if (!perMinuteLimit) {
    return {
      ...
      error: 'pills_update_rate_limit_per_minute is not configured',
    };
  }
  ...
  const keyFingerprint =
    typeof options.keyFingerprint === 'string' ? options.keyFingerprint : 'unknown';
  const rateKey =
    phase === 'ip'
...
export async function handleAdminPills(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'PATCH') {
    if (Array.isArray(body.items)) {
      ...
      for (let i = 0; i < body.items.length; i += 1) {
        ...
        reorderStatements.push(
          db
            .prepare(`UPDATE pills SET sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
            .bind(i, item.id),
        );
      }
    ...
    let nextValueSecondary =
      existing.value_secondary == null ? null : Number(existing.value_secondary);
    let nextGraphEmbedUrl =
      typeof existing.graph_embed_url === 'string' ? existing.graph_embed_url : '';
    let nextGraphPayloadJson =
      typeof existing.graph_payload_json === 'string' ? existing.graph_payload_json : '';
    ...
    if (Object.hasOwn(body, 'value')) {
      ...
      if (!Number.isFinite(next))
        return jsonResponse({ error: 'value must be a number' }, 400, corsHeaders);
      updates.push('value = ?');
      values.push(next);
    }
    ...
    if (Object.hasOwn(body, 'valueMode')) {
      const mode = normalizePillMode(body.valueMode);
      if (!mode)
        return jsonResponse(
          { error: 'valueMode must be number, percentage, agree_disagree, or graph_embed' },
          400,
          corsHeaders,
        );
      nextMode = mode;
    ...
    if (Object.hasOwn(body, 'valueSecondary')) {
      const next =
        body.valueSecondary == null || body.valueSecondary === ''
          ? null
          : Number(body.valueSecondary);
      if (next != null && !Number.isFinite(next))
        return jsonResponse({ error: 'valueSecondary must be a number' }, 400, corsHeaders);
      nextValueSecondary = next;
      updates.push('value_secondary = ?');
      values.push(next);
    }
    if (Object.hasOwn(body, 'graphEmbedUrl')) {
      const next =
        body.graphEmbedUrl == null ? null : String(body.graphEmbedUrl).trim().slice(0, 2048);
      if (next && !isSafeHttpUrl(next)) {
        return jsonResponse(
          { error: 'graphEmbedUrl must be a valid http or https URL' },
          400,
          corsHeaders,
        );
      }
    ...
    if (Object.hasOwn(body, 'graphPayloadJson')) {
      const next =
        body.graphPayloadJson == null ? null : String(body.graphPayloadJson).trim().slice(0, 10000);
      if (next) {
        try {
          JSON.parse(next);
        } catch {
    ...
    if (
      nextMode === 'agree_disagree' &&
      (nextValueSecondary == null || !Number.isFinite(nextValueSecondary))
    ) {
      return jsonResponse(
        { error: 'agree_disagree pills require valueSecondary' },
        400,
        corsHeaders,
      );
    }
    if (nextMode === 'graph_embed' && !nextGraphEmbedUrl && !nextGraphPayloadJson) {
      return jsonResponse(
        { error: 'graph_embed pills require graphEmbedUrl or graphPayloadJson' },
        400,
        corsHeaders,
      );
    }
...
function normalizePillMode(
  rawMode: any,
): 'number' | 'percentage' | 'agree_disagree' | 'graph_embed' | null {
  const next = String(rawMode ?? 'number')
    .trim()
    .toLowerCase();
  if (
    next === 'number' ||
...
function normalizePillInput(body: any, options: { allowImageUploadUrl: boolean }) {
  ...
  const valueSecondary =
    body?.valueSecondary == null || body.valueSecondary === '' ? null : Number(body.valueSecondary);
  if (valueSecondary != null && !Number.isFinite(valueSecondary))
    return { ok: false as const, error: 'valueSecondary must be a number' };
  const graphEmbedUrl =
    typeof body?.graphEmbedUrl === 'string' ? body.graphEmbedUrl.trim().slice(0, 2048) : '';
  ...
  const graphPayloadJson =
    typeof body?.graphPayloadJson === 'string' ? body.graphPayloadJson.trim().slice(0, 10000) : '';
...
export async function handleAdminPillImageUpload(request: any, env: any, corsHeaders: any) {
  ...
  if (bytes.byteLength > 5 * 1024 * 1024)
    return jsonResponse({ error: 'Image too large (max 5MB)' }, 413, corsHeaders);
  const ext =
    file.type === 'image/png'
      ? 'png'
...
export async function handleAdminPillsSettings(request: any, env: any, corsHeaders: any) {
  ...
  if (envKey) {
    return jsonResponse(
      { error: 'PILLS_API_KEY is managed by environment secret' },
      409,
      corsHeaders,
    );
  }
  ...
  return jsonResponse(
    { ok: true, hasKey: true, maskedKey: buildMaskedKey(nextHash) },
    200,
    corsHeaders,
  );
}
...
async function hashPillsApiKey(rawKey: any) {
  ...
  const derived = new Uint8Array(
    await derivePbkdf2Sha256(normalized, salt, PILLS_KEY_HASH_ITERATIONS),
  );
...
function parseUsersListQuery(url: any) {
  ...
  const subscription =
    typeof q.get('subscription') === 'string' ? q.get('subscription').trim().toLowerCase() : 'all';
  return { page, pageSize, search, role, subscription };
}

function usersListWhereClause({ search, role, subscription }: any) {
  const clauses = ['1=1'];
  const binds = [];
  if (search) {
...
export async function handleAdminUsers(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'GET') {
    ...
    const { sql: whereSql, binds: whereBinds } = usersListWhereClause({
      search,
      role,
      subscription,
    });
  ...
  const target = await db
    ...
    .first();
  if (!target)
    return jsonResponse({ error: 'User not found', code: 'not_found' }, 404, corsHeaders);
  ...
  const wantsSubscription =
    ...
    (body?.createSubscription && typeof body.createSubscription === 'object');
  ...
  if (wantsSubscription) {
    if (actorRole === 'admin' && target.role === 'super_admin') {
      return jsonResponse(
        { error: 'Only super_admin may edit super_admin accounts', code: 'forbidden_target' },
        403,
        corsHeaders,
      );
    }
    ...
    const statusRaw =
      typeof body?.subscriptionStatus === 'string'
        ...
        : typeof body?.createSubscription?.status === 'string'
          ? body.createSubscription.status
          : '';
    ...
    if (transition.next === 'none') {
      if (!latest?.id) {
        return jsonResponse(
          { error: 'User has no subscription to cancel', code: 'no_subscription' },
          400,
          corsHeaders,
        );
      }
      ...
      try {
        ...
        return jsonResponse(
          { error: 'Update failed', code: 'transaction_failed' },
          500,
          corsHeaders,
        );
      }
    ...
    if (!latest?.id) {
      ...
      const planTypeRaw =
        typeof createSub?.planType === 'string'
          ? createSub.planType
...
export async function handleAdminUserImportCsv(request: any, env: any, corsHeaders: any) {
  ...
  const maxEmails =
    Number.isFinite(configuredMaxEmails) && configuredMaxEmails > 0 ? configuredMaxEmails : 10000;
  const importPlan =
    typeof rawImportPlan === 'string' && rawImportPlan.trim() ? rawImportPlan.trim() : 'monthly';
  ...
  if (requirePurchaseId && rows.some((row) => !row.purchaseId)) {
    return jsonResponse(
      {
        error:
          'Every CSV row must include a purchaseId (or clientId) when requirePurchaseId is enabled',
  ...
  for (const row of rows) {
    ...
    if (row.purchaseId) {
      withPurchaseId += 1;
      statements.push(
        subUpsertLegacy.bind(subscriptionId, userId, importPlan, importCustomerRef, row.purchaseId),
      );
    } else {
...
export async function handleAdminAnalytics(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'PATCH') {
    const body = await request.json().catch(() => null);
    const viewCounting =
      body?.viewCounting && typeof body.viewCounting === 'object' ? body.viewCounting : null;
    const toIntString = (value: any, fallback: number) => {
      const next = Number.parseInt(String(value ?? ''), 10);
      return String(Number.isFinite(next) && next >= 0 ? next : fallback);
    };
    const updates: [string, string][] = [];
    if (viewCounting && Object.hasOwn(viewCounting, 'minSegmentsPerSession')) {
      updates.push([
        'analytics_view_min_segments',
        toIntString(viewCounting.minSegmentsPerSession, 1),
      ]);
    }
    if (viewCounting && Object.hasOwn(viewCounting, 'minWatchSeconds')) {
      updates.push([
        'analytics_view_min_watch_seconds',
        toIntString(viewCounting.minWatchSeconds, 15),
      ]);
    }
  ...
  const settingKeys = ['analytics_view_min_segments', 'analytics_view_min_watch_seconds'] as const;
...
function parseAnalyticsQuery(url: URL): { options: AnalyticsQueryOptions; error?: string } {
  ...
  if (!ranges.includes(range))
    return {
      ...
      error: 'range must be one of 7d, 30d, 90d, 180d, 365d',
    };
  ...
  if (format === 'csv' && dataset === 'all') {
    return {
      options: fallbackAnalyticsOptions(),
      error: 'dataset must be specified when format=csv',
    };
  }
...
export function computeEngagementScore(
  ...
  const completion =
    completionRatePercent == null ? 0 : Math.max(0, Math.min(100, completionRatePercent));
...
function buildHeatmapSeries(
  rows: Array<{ bucket_pct: number; watch_seconds: number; segment_hits: number }>,
  bucketCount = 100,
) {
...
export async function buildSegmentAnalyticsSnapshotWithOptions(
  db: any,
  env: any,
  ...
  const [
    ...
    `)
      .bind(startAt, minSegmentsPerView, minWatchSecondsPerView)
      .first(),
    db
      .prepare(`
      WITH session_rollup AS (
    ...
    `)
      .bind(startAt, minSegmentsPerView, minWatchSecondsPerView, startAt)
      .all(),
    db
      .prepare(`
  ...
  if (options.videoId) {
    ...
    `)
      .bind(options.videoId, startAt)
      .all();
    const buckets = buildHeatmapSeries(
      (heatmapRows?.results ?? []).map((row: any) => ({
  ...
  const subscriptionTrendsMap = new Map<
    string,
    {
      bucket: string;
  ...
  const subscriptionTrends = Array.from(subscriptionTrendsMap.values()).sort((a, b) =>
  ...
  const activeMrrEstimateEur = (planBreakdownRows?.results ?? []).reduce(
    (sum: number, row: any) => {
      const rawPlan = typeof row.plan_type === 'string' ? row.plan_type : 'monthly';
      const plan: keyof typeof monthlyPriceByPlan =
        rawPlan === 'yearly' || rawPlan === 'club' ? rawPlan : 'monthly';
  ...
  const cashflowTrend = subscriptionTrends.map((row) => {
    ...
    return {
      ...
      estimatedNetNewEur: Number(
        (estimatedNewRevenueEur - row.churnedSubscriptions * monthlyBasePrice).toFixed(2),
      ),
    };
  });
  ...
  const totalChurnedSubscriptions = subscriptionTrends.reduce(
    (sum, row) => sum + row.churnedSubscriptions,
    0,
  );
  const churnRate =
    totalNewSubscriptions > 0
      ? Number(((totalChurnedSubscriptions / totalNewSubscriptions) * 100).toFixed(2))
      : 0;
  const averageRetentionPercent = Number(
    Number(globalRetentionRow?.average_retention_percent ?? 0).toFixed(2),
  );

  const videoStats = (videoStatsRows?.results ?? []).map((row: any) => {
    const averageRetentionPercent =
      row.average_retention_percent == null
        ? null
        : Number(Number(row.average_retention_percent).toFixed(2));
    const completionRatePercent =
      row.completion_rate_percent == null
        ? null
  ...
  const kpis = {
    ...
    activeSubscribers: Number(
      (planBreakdownRows?.results ?? []).reduce(
  ...
  const definitions = {
    ...
    averageRetentionPercent:
      'Average max watch-through percent across qualified sessions in the selected range.',
    ...
    estimatedActiveMrrEur:
      'Approximate monthly recurring revenue from active/trialing users using admin configured prices.',
    engagementScore:
      'Video-level score from 0–100 based on retention depth and completion rate (sessions reaching 90%+ watch-through).',
    countryViews:
...
function buildAnalyticsCsvExport(snapshot: any, dataset: AnalyticsDataset) {
  ...
  if (dataset === 'retention') {
    ...
    rows.push(
      'video_id,title,view_count,total_watch_seconds,average_retention_percent,engagement_score',
    );
    for (const row of snapshot.videoStats ?? []) {
      rows.push(
        `${escapeCsvCell(row.videoId)},${escapeCsvCell(row.title)},${escapeCsvCell(row.viewCount)},${escapeCsvCell(row.totalWatchSeconds ?? 0)},${escapeCsvCell(row.averageRetentionPercent ?? '')},${escapeCsvCell(row.engagementScore ?? '')}`,
      );
    }
    return rows.join('\n');
  }
  ...
  if (dataset === 'countries') {
    ...
    for (const country of Array.from(countries).sort()) {
      const viewsRow = (snapshot.countries?.views ?? []).find(
        (row: any) => String(row.country) === country,
      );
      rows.push(
        `${escapeCsvCell(country)},${escapeCsvCell(viewsRow?.uniqueSessions ?? 0)},${escapeCsvCell(watchByCountry.get(country) ?? 0)}`,
      );
    }
    return rows.join('\n');
  }
  if (dataset === 'subscriptions') {
    ...
    for (const row of snapshot.subscriptionOverview?.trends ?? []) {
      rows.push(
        `${escapeCsvCell(row.bucket)},${escapeCsvCell(row.newSubscriptions)},${escapeCsvCell(row.churnedSubscriptions)},${escapeCsvCell(row.expiringSubscriptions)}`,
      );
    }
    return rows.join('\n');
  }
  if (dataset === 'cashflow') {
    ...
    for (const row of snapshot.cashflow?.trend ?? []) {
      rows.push(
        `${escapeCsvCell(row.bucket)},${escapeCsvCell(row.estimatedNewRevenueEur)},${escapeCsvCell(row.estimatedNetNewEur)}`,
      );
    }
    return rows.join('\n');
  }
  ...
  for (const key of Object.keys(snapshot.kpis ?? {})) {
    rows.push(
      `${escapeCsvCell(key)},${escapeCsvCell(snapshot.kpis[key])},${escapeCsvCell(snapshot.definitions?.[key] ?? '')}`,
    );
  }
  return rows.join('\n');
}
...
const SEARCH_TRAFFIC_HOST_MARKERS = [
  'google.',
  'bing.',
  'duckduckgo.',
  'search.yahoo.',
...
const SOCIAL_TRAFFIC_HOST_MARKERS = [
  'facebook.',
  'instagram.',
  'x.com',
  't.co',
...
export function derivePlaybackPositionSeconds(payload: any) {
  ...
  const segmentDuration = Number.isFinite(payload?.segmentDurationSeconds)
    ? Number(payload.segmentDurationSeconds)
    : null;
  ...
  if (Number.isFinite(payload?.positionSeconds))
    return Math.max(0, Number(payload.positionSeconds));
