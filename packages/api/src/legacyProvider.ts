...
function legacyFetchTimeoutMs(env: LegacyEnv): number {
  ...
  const parsed =
    raw != null && String(raw).trim() !== ''
      ...
      : DEFAULT_LEGACY_FETCH_TIMEOUT_MS;
...
export function isLegacyFetchTimeout(err: unknown): boolean {
  ...
  return Boolean(
    err && typeof err === 'object' && (err as { code?: string }).code === 'legacy_timeout',
  );
}

async function legacyFetch(env: LegacyEnv, url: string, init: RequestInit): Promise<Response> {
...
export function getLegacyValidationApiBase(
  env: LegacyEnv,
...
async function legacyPost<T = Record<string, unknown>>(
  ...
  if (!result.ok) {
    const message =
      typeof (result.parsed as { message?: string })?.message === 'string'
...
export function buildLegacyOrderBody(
  env: LegacyEnv,
  input: LegacyCreateOrderInput,
...
export function interpretLegacyValidationResponse(
  ...
  if (status === 400 && (reason === 'cardOnFile' || message.toLowerCase().includes('cardonfile'))) {
...
export async function getLegacyOrder(env: LegacyEnv, idOrder: string) {
  ...
  return legacyGet(
    env,
    base,
    `/order/${encodeURIComponent(idMerchant)}/${encodeURIComponent(idOrder)}`,
  );
}
