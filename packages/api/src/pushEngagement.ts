...
export function tierDelaySeconds(
  tier: string,
  settings: Awaited<ReturnType<typeof getPushTierSettings>>,
) {
...
export async function handlePushEvents(
  request: Request,
  env: any,
  ...
  if (!deliveryId) {
    return jsonResponse(
      { error: 'deliveryId is required', code: 'invalid_payload' },
      400,
      corsHeaders,
    );
  }
  ...
  if (eventType === 'click') {
    ...
    if (!delivery || delivery.status !== 'sent' || !delivery.sent_at) {
      return jsonResponse(
        { error: 'Delivery not found or not sent', code: 'not_found' },
        404,
        corsHeaders,
      );
    }
    const existing = await db
      .prepare(`SELECT id FROM push_clicks WHERE delivery_id = ?`)
      .bind(deliveryId)
      .first();
    if (existing) {
  ...
  if (eventType === 'session_start') {
    ...
    if (!originVideoId) {
      return jsonResponse(
        { error: 'originVideoId is required', code: 'invalid_payload' },
        400,
        corsHeaders,
      );
    }
    ...
    `)
      .bind(sessionId, delivery.campaign_id, deliveryId, delivery.user_id, originVideoId)
      .run();
  ...
  return jsonResponse(
    { error: 'Unsupported event type', code: 'invalid_payload' },
    400,
    corsHeaders,
  );
}

export async function handleAdminPushAnalytics(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  if (!campaignId) {
    return jsonResponse(
      { error: 'campaignId is required', code: 'invalid_query' },
      400,
      corsHeaders,
    );
  }
  ...
  const [sentRow, clickRow, latencyRow, watchRow, completionRow, depthRow] = await Promise.all([
    db
      .prepare(
        `SELECT COUNT(*) AS count FROM push_deliveries WHERE campaign_id = ? AND status = 'sent'`,
      )
      .bind(campaignId)
      .first(),
    db
      .prepare(`SELECT COUNT(*) AS count FROM push_clicks WHERE campaign_id = ?`)
      .bind(campaignId)
      .first(),
    db
      .prepare(`
    ...
    `)
      ...
      .first(),
    db
      .prepare(`SELECT COUNT(*) AS count FROM push_watch_sessions WHERE campaign_id = ?`)
      .bind(campaignId)
      .first(),
    db
      .prepare(`
  ...
  return jsonResponse(
    {
      ...
      funnel: {
        ...
        medianClickLatencySeconds:
          latencyRow?.median_latency != null
        ...
        completionRatePercent:
          watchSessions > 0 ? Number(((completions / watchSessions) * 100).toFixed(2)) : 0,
        avgSessionDepth:
          depthRow?.avg_depth != null ? Number(Number(depthRow.avg_depth).toFixed(2)) : 0,
      },
    },
    200,
    corsHeaders,
...
export async function claimPushDeliveryForQueue(db: any, deliveryId: string) {
  ...
  `)
    .bind(deliveryId, `-${STALE_PUSH_QUEUE_CLAIM_HOURS} hours`)
    .run();
...
export async function enqueueOverduePushDeliveries(env: any) {
  ...
  const rows = await db
    ...
    LIMIT 100
  `)
    .bind(`-${STALE_PUSH_QUEUE_CLAIM_HOURS} hours`)
    .all();
...
export async function createPushCampaignAndDeliveries(options: {
  ...
  `)
    .bind(campaignId, videoId, createdByUserId, mode)
    .run();
  ...
  for (const sub of subscriptions as any[]) {
    ...
    `)
      .bind(deliveryId, campaignId, sub.user_id, sub.subscription_id, scheduledAt, delaySeconds)
      .run();
...
function assignTierFromMedianLatency(
...
export async function syncPushEngagementProfiles(env: any) {
  ...
  for (const userId of userIds) {
    ...
    const campaignsObserved = Number(
      (
        await db
          .prepare(`
    ...
    let tier = 'unknown';
    if (
      campaignsObserved >= settings.minCampaignsUnknown &&
      latencies.length === 0 &&
      campaignsObserved >= settings.dormantCampaigns
    ) {
      tier = 'dormant';
