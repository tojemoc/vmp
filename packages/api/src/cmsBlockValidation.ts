...
export function parseCmsBlock(raw: unknown): CmsBlock | null {
  ...
  switch (raw.type) {
    case 'rich_text':
      return isRichTextDocument(raw.content) ? { type: 'rich_text', content: raw.content } : null;
    case 'image': {
      ...
      return typeof raw.caption === 'string' ? { ...block, caption: raw.caption } : null;
    }
    case 'callout':
      return CALLOUT_VARIANTS.has(raw.variant as CmsCalloutVariant) &&
        isRichTextDocument(raw.content)
        ? { type: 'callout', variant: raw.variant as CmsCalloutVariant, content: raw.content }
        : null;
    case 'divider':
      return { type: 'divider' };
    case 'table': {
      if (!Array.isArray(raw.columns) || !Array.isArray(raw.columnKeys) || !Array.isArray(raw.rows))
        return null;
