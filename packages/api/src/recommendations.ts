...
export function scoreRecommendationVideos(
  ...
  const filtered =
    settings.categoryLock && currentCategory
  ...
  const timestamps = filtered.map(
    (v) => parseTimestamp(v.published_at) || parseTimestamp(v.upload_date),
  );
  ...
  const scored = filtered.map((video, index) => {
    ...
    const lowViewScore = settings.lowViewsBoost > 0 ? (maxViews - views) / viewSpread : 0;
...
export async function handleVideoRecommendations(
  request: Request,
  env: any,
