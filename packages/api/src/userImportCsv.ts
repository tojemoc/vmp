...
function parseStructuredCsvRows(csvText: string, maxRows: number): CsvUserImportRow[] {
  ...
  const resolvedPurchaseIdIndex = hasHeader ? purchaseIdIndex : firstCells.length > 1 ? 1 : -1;

  const firstEmailCandidate = firstCells[0];
  if (
    !hasHeader &&
    firstCells.length === 1 &&
    (!firstEmailCandidate || !EMAIL_REGEX.test(firstEmailCandidate))
  ) {
    return [];
  }
  ...
  for (const line of dataLines) {
    ...
    const emailRaw = String(cells[resolvedEmailIndex] ?? '')
      .trim()
      .toLowerCase();
    ...
    const purchaseIdRaw =
      resolvedPurchaseIdIndex >= 0 ? String(cells[resolvedPurchaseIdIndex] ?? '').trim() : '';
    rows.push({
      email: emailRaw,
