...
async function hkdfExpand(prk: Uint8Array, info: Uint8Array, length: number): Promise<Uint8Array> {
  ...
  const t1 = new Uint8Array(
    await crypto.subtle.sign(
      'HMAC',
      key,
      toArrayBuffer(concatBuffers(info, new Uint8Array([0x01]))),
    ),
  );
...
export async function sendPushNotification(
  ...
  try {
    ...
    throw Object.assign(new Error(`Push fetch error: ${e.message}`), {
      code: 'push_failed',
      statusClass: 'network_error',
      endpointHost,
    });
  ...
  if (!response.ok) {
    ...
    if (response.status === 410 || response.status === 404) {
      ...
      throw Object.assign(new Error('Push subscription expired'), {
    ...
    throw Object.assign(new Error(`Push delivery failed: ${response.status}`), {
      code: 'push_failed',
      status: response.status,
      statusClass,
      responseSnippet,
...
export async function sendPushToAllSubscribers(
  ...
  if (staleEndpoints.length > 0) {
    ...
    await db
      ...
      .bind(...staleEndpoints)
      .run()
      .catch((e: unknown) => console.error('Cleanup error:', e));
  }
