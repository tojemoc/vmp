...
export function clampNewsletterPollIntervalMs(raw: any) {
  ...
  if (!Number.isFinite(n) || n < 60_000 || n > 86_400_000)
    return DEFAULT_NEWSLETTER_POLL_INTERVAL_MS;
  return n;
}
...
function newsletterLog(event: any, fields = {}) {
  console.log(
    JSON.stringify({ service: 'newsletter', source: 'brevo_newsletter', event, ...fields }),
  );
}
...
function brevoTimeoutMs(env: any) {
  ...
  const n =
    raw != null && String(raw).trim() !== '' ? Number.parseInt(String(raw).trim(), 10) : 5000;
 ...
 *
 * Newsletter list = active/trialing paying subscribers who have not opted out.
 * Cancellation / non-paying status removes list membership. Opt-out removes
 * membership even while paying. Transactional mail is unrelated to this list.
 */
...
/**
 * Add or update a paying subscriber on the Brevo marketing list.
 *
 * No-ops (returns false) when the user has opted out of the newsletter, and
 * honors Brevo's suppression state so a Brevo unsubscribe is never silently
 * overwritten. Callers should only invoke this for active/trialing subscribers;
 * use removeSubscriberFromNewsletter when they opt out or stop paying.
 * Returns true only when the remote add succeeds.
 */
export async function syncPayingSubscriberToNewsletter(
  ...
  const row = await db
    .prepare('SELECT email, newsletter_opted_out_at FROM users WHERE id = ? LIMIT 1')
    .bind(userId)
    .first();
  ...
  if (!res.ok) {
    const err = asRecord(await res.json().catch(() => null));
    const userIdHash = await hashUserId(userId);
    newsletterLog('sync_contact_failed', {
      userIdHash,
      status: res.status,
      code: recordString(err, 'code'),
    });
...
async function syncAllEligibleSubscribers(db: any, env: any) {
  ...
  // Paying + not opted out. Reconcile also removes opted-out or non-paying
  // contacts that a prior sync left on the list.
  const eligibleRows = await db
    ...
    WHERE s.status IN ('active', 'trialing')
      ...
      AND u.newsletter_opted_out_at IS NULL
  `)
    .all();
  ...
  if (eligibleUserIds.length) {
    ...
    for (let i = 0; i < eligibleUserIds.length; i += BATCH_SIZE) {
      const chunk = eligibleUserIds.slice(i, i + BATCH_SIZE);
  ...
  const pageLimit = 500;
  while (true) {
    const pageRes = await brevoFetch(
      `/contacts/lists/${listId}/contacts?limit=${pageLimit}&offset=${offset}`,
      { method: 'GET' },
      env,
    );
    if (!pageRes.ok) break;
    const page = asRecord(await pageRes.json().catch(() => null));
  ...
  for (let i = 0; i < staleEmails.length; i += BREVO_BATCH_SIZE) {
    ...
    if (!res.ok && res.status !== 404) {
      const err = asRecord(await res.json().catch(() => null));
      newsletterLog('remove_stale_email_failed', {
        status: res.status,
        code: recordString(err, 'code'),
      });
    }
...
async function ensureBrevoNewsletterSendsTable(db: any) {
  ...
  if (!(await brevoNewsletterSendsHasColumn(db, 'in_flight'))) {
    await db
      .prepare('ALTER TABLE brevo_newsletter_sends ADD COLUMN in_flight INTEGER NOT NULL DEFAULT 0')
      .run();
  }
 ...
 */
async function persistCampaignIdForDedupeKey(
  db: any,
  dedupeKey: any,
  newId: any,
  correlationId: any,
  ...
  newsletterLog('send_campaign_id_persist_failed', {
    correlationId,
    dedupeKeyLen: dedupeKey.length,
  });
...
function jsonResponse(data: any, status = 200, corsHeaders = {}) {
  const safePayload = status >= 500 ? { error: 'Internal error', code: 'internal_error' } : data;
...
export async function handleAdminNewsletterSettings(request: any, env: any, corsHeaders: any) {
  ...
  if (request.method === 'GET') {
    const [brevoSubscriberListId, brevoCampaignSenderEmail, brevoCampaignSenderName, pollRaw] =
      await Promise.all([
  ...
  if ('brevoSubscriberListId' in body) {
    ...
    if (v === null || v === '') {
      ...
      if (!/^\d+$/.test(t)) {
        return jsonResponse(
          {
            error: 'brevoSubscriberListId must be a positive integer or empty',
  ...
  if ('brevoCampaignSenderEmail' in body) {
    ...
    if (v === null || v === '') {
      ...
      if (!EMAIL_RE.test(t)) {
        return jsonResponse(
          { error: 'Invalid brevoCampaignSenderEmail', code: 'invalid_sender_email' },
          400,
          corsHeaders,
        );
      }
  ...
  if ('brevoNewsletterPollIntervalMs' in body) {
    ...
    if (v === null || v === '') {
      ...
      return jsonResponse(
        {
          error: 'brevoNewsletterPollIntervalMs must be a number (milliseconds)',
          code: 'invalid_poll_interval',
        },
        400,
        corsHeaders,
...
export async function handleAdminNewsletterSend(request: any, env: any, corsHeaders: any) {
  ...
  if (!env.BREVO_API_KEY) {
    return jsonResponse(
      { error: 'Brevo is not configured (missing BREVO_API_KEY)', code: 'brevo_not_configured' },
      503,
      corsHeaders,
    );
  }
  ...
  if (templateId) {
    const template = await getDb(env)
      .prepare('SELECT subject, html_body FROM newsletter_templates WHERE id = ? LIMIT 1')
      .bind(templateId)
      .first();
    if (!template) {
      return jsonResponse(
  ...
  if (!subject || !htmlBody.trim()) {
    return jsonResponse(
      { error: 'subject and htmlBody are required', code: 'validation' },
      400,
      corsHeaders,
    );
  }
  if (!dedupeKey || dedupeKey.length > 256) {
    return jsonResponse(
      { error: 'dedupeKey is required (non-empty string, max 256 characters)', code: 'validation' },
      400,
      corsHeaders,
    );
  }
  ...
  const loadSendRow = () => db.prepare(sendRowSql).bind(dedupeKey).first();
  ...
  await db
    .prepare('INSERT OR IGNORE INTO brevo_newsletter_sends (dedupe_key) VALUES (?)')
    .bind(dedupeKey)
    .run();
  ...
  if (isNewsletterSendFinished(row)) {
    newsletterLog('send_idempotent_after_stale', {
      correlationId,
      campaignId: Number(row.campaign_id),
    });
    return jsonResponse(
  ...
  try {
    ...
    return jsonResponse(
      { error: 'Failed to load newsletter configuration', code: 'config_error' },
      500,
      corsHeaders,
    );
  }
  ...
  let campaignId =
    existingCampaignId != null && Number.isFinite(existingCampaignId) ? existingCampaignId : null;
  ...
  try {
    if (campaignId == null || !Number.isFinite(campaignId)) {
      const createRes = await brevoFetch(
        '/emailCampaigns',
        { method: 'POST', body: JSON.stringify(campaignPayload) },
        env,
      );
      if (!createRes.ok) {
        const err = asRecord(await createRes.json().catch(() => null));
        const brevoCode = recordString(err, 'code');
        const brevoMessage = recordString(err, 'message');
        ...
        const status =
          createRes.status === 504
            ...
            : createRes.status >= 400 && createRes.status < 600
              ? createRes.status
              : 502;
        const msg = brevoMessage || 'Failed to create email campaign';
        await releaseIfHeld();
        return jsonResponse(
          {
            error: msg,
      ...
      if (newId == null || !Number.isFinite(Number(newId))) {
        ...
        await releaseIfHeld();
        return jsonResponse(
          { error: 'Unexpected response creating campaign', code: 'brevo_campaign_error' },
          502,
          corsHeaders,
        );
      }
    ...
    if (!sendRes.ok && sendRes.status !== 204) {
      const err = asRecord(await sendRes.json().catch(() => null));
      const brevoCode = recordString(err, 'code');
      const brevoMessage = recordString(err, 'message');
      ...
      const status =
        sendRes.status === 504
          ? 504
          : sendRes.status >= 400 && sendRes.status < 600
            ? sendRes.status
            : 502;
      const msg = brevoMessage || 'Campaign created but send failed';
...
export async function handleAdminNewsletterCampaigns(request: any, env: any, corsHeaders: any) {
  ...
  if (!res.ok) {
    ...
    return jsonResponse(
      {
        error: brevoMessage || 'Failed to fetch campaigns',
  ...
  newsletterLog('campaigns_list_ok', { correlationId, count: campaigns.length });
...
export async function handleAdminNewsletterTemplates(request: any, env: any, corsHeaders: any) {
  ...
  if (!name || !subject || !htmlBody.trim()) {
    return jsonResponse(
      { error: 'name, subject and htmlBody are required', code: 'validation' },
      400,
      corsHeaders,
    );
  }
 ...
 */
export async function handleAdminNewsletterTemplateById(
  request: any,
  env: any,
  corsHeaders: any,
  templateId: any,
  ...
  if ('name' in body) {
    if (typeof body.name !== 'string' || !body.name.trim()) {
      return jsonResponse(
        { error: 'name must be a non-empty string', code: 'validation' },
        400,
        corsHeaders,
      );
    }
  ...
  if ('subject' in body) {
    if (typeof body.subject !== 'string' || !body.subject.trim()) {
      return jsonResponse(
        { error: 'subject must be a non-empty string', code: 'validation' },
        400,
        corsHeaders,
      );
    }
  ...
  if ('htmlBody' in body) {
    if (typeof body.htmlBody !== 'string' || !body.htmlBody.trim()) {
      return jsonResponse(
        { error: 'htmlBody must be a non-empty string', code: 'validation' },
        400,
        corsHeaders,
      );
    }
  ...
  if (!updates.length) {
    return jsonResponse(
      { error: 'Provide at least one of: name, subject, htmlBody', code: 'validation' },
      400,
      corsHeaders,
    );
  }
 ...
 */
export async function handleAdminNewsletterDrafts(
  request: any,
  env: any,
  corsHeaders: any,
  draftId?: string,
  ...
  if (draftId) {
    if (request.method === 'PATCH') {
      ...
      if (typeof body?.subject === 'string') {
        if (!body.subject.trim()) {
          return jsonResponse(
            { error: 'subject cannot be empty', code: 'validation' },
            400,
            corsHeaders,
          );
        }
      ...
      if (typeof body?.htmlBody === 'string') {
        if (!body.htmlBody.trim()) {
          return jsonResponse(
            { error: 'htmlBody cannot be empty', code: 'validation' },
            400,
            corsHeaders,
          );
        }
      ...
      values.push(draftId);
      const run = await db
        .prepare(`UPDATE newsletter_drafts SET ${updates.join(', ')} WHERE id = ?`)
        .bind(...values)
        .run();
      const changed = run.meta?.changes ?? run.changes ?? 0;
      if (!changed)
      ...
      const row = await db
        .prepare('SELECT * FROM newsletter_drafts WHERE id = ? LIMIT 1')
        .bind(draftId)
        .first();
    ...
    if (request.method === 'DELETE') {
      const run = await db
        .prepare('DELETE FROM newsletter_drafts WHERE id = ?')
        .bind(draftId)
        .run();
      const changed = run.meta?.changes ?? run.changes ?? 0;
      if (!changed)
  ...
  if (request.method === 'POST') {
    ...
    await db
      ...
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `)
      .bind(id, name, subject, htmlBody, scheduledAt)
      .run();
    const row = await db
      .prepare('SELECT * FROM newsletter_drafts WHERE id = ? LIMIT 1')
      .bind(id)
...
export async function handleAdminNewsletterSchedule(request: any, env: any, corsHeaders: any) {
  ...
  if (!subject || !htmlBody.trim() || !scheduledAt) {
    return jsonResponse(
      { error: 'subject, htmlBody, and scheduledAt are required', code: 'validation' },
      400,
      corsHeaders,
    );
  }
  ...
  if (draftId) {
    const draft = await db
      .prepare('SELECT id FROM newsletter_drafts WHERE id = ? LIMIT 1')
      .bind(draftId)
      .first();
    if (!draft?.id) {
      return jsonResponse({ error: 'Draft not found', code: 'not_found' }, 404, corsHeaders);
  ...
  const scheduleRes = await brevoFetch(
    `/emailCampaigns/${campaignId}/schedule`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  if (!scheduleRes.ok && scheduleRes.status !== 204) {
    ...
    return jsonResponse(
      ...
      scheduleRes.status >= 400 && scheduleRes.status < 600 ? scheduleRes.status : 502,
      corsHeaders,
    );
  }
