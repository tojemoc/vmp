...
export class CmsPagesRepository {
  ...
  async getPageById(id: string): Promise<CmsPage | null> {
    const row = await this.db
      .prepare(`SELECT * FROM cms_pages WHERE id = ?`)
      .bind(id)
      .first<CmsPageRow>();
  ...
  async updatePage(
    ...
    await this.db
      ...
      .bind(title, slug, description, status, serializeCmsBlocks(content), now, publishedAt, id)
      .run();
  ...
  async deletePage(id: string): Promise<boolean> {
    const result = (await this.db.prepare(`DELETE FROM cms_pages WHERE id = ?`).bind(id).run()) as {
      meta?: { changes?: number };
    };
