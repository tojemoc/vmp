...
export async function handleSiteFooterPublic(
  request: Request,
  env: any,
  ...
  try {
    ...
    const content: CmsBlock[] = footerPage?.status === 'published' ? footerPage.content : [];
...
export async function handleSiteFooterAdmin(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  if (request.method === 'PATCH') {
    ...
    if (!Array.isArray(raw.linkPageIds)) {
      return jsonResponse(
        { error: 'linkPageIds must be an array', code: 'INVALID_LINKS' },
        400,
        corsHeaders,
      );
    }
