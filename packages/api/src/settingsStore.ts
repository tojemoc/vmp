...
function normalizeCacheTtlSeconds(key: string, ttlSeconds: number) {
  if (key.startsWith(STRIPE_SETTING_PREFIX))
    return Math.min(Math.max(1, ttlSeconds), STRIPE_SETTING_TTL_SECONDS);
  ...
  if (missingKeys.length) {
    try {
      ...
      const rowsByKey = new Map(
        (rows?.results ?? []).map((row: any) => [String(row.key), row.value]),
      );
