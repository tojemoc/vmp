...
async function getOfflineSettings(env: any): Promise<OfflineSettings> {
  const [maxLicenseDaysRaw, revalidationDaysRaw, deviceLimitDefaultRaw, deviceLimitClubRaw] =
    await Promise.all([
...
export async function handleRegisterOfflineDevice(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  if (!isAdministrativeRole(user.role)) {
    ...
    if (!subscription) {
      return errorResponse(
        'Active subscription required for offline downloads',
        403,
        corsHeaders,
    ...
    `)
      .bind(deviceId, user.sub, deviceName, publicKey, deviceTokenHash, user.sub, limit)
      .run();
  ...
  `)
    .bind(deviceId, user.sub, deviceName, publicKey, deviceTokenHash)
    .run();
...
export async function handleListOfflineDevices(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
...
async function resolveAvailableOfflineR2Reader(
  env: any,
  videoId: string,
): Promise<OfflineR2Reader | null> {
...
export async function handleAuthorizeDownload(
  ...
  if (device.user_id !== user.sub) {
    return errorResponse(
      'Device token does not match authenticated user',
      403,
      corsHeaders,
  ...
  if (deviceId !== device.id) {
    return errorResponse(
      'deviceId does not match device token',
      403,
      corsHeaders,
      'device_id_mismatch',
    );
  ...
  const hasPremium =
    isAdministrativeRole(user.role) || Boolean(await getActiveSubscription(db, user.sub));
  if (!hasPremium) {
  ...
  const licenseRow = await db
    ...
    LIMIT 1
  `)
    .bind(user.sub, resolvedVideoId, rendition, deviceId)
    .first();
  ...
  const estimatedBytes =
    manifest.totalBytes > 0 ? manifest.totalBytes : estimateDownloadBytes(durationSec, rendition);
...
export async function handleDownloadAsset(
  ...
  if (!dt)
    return errorResponse('Missing download token', 403, corsHeaders, 'missing_download_token');
  ...
  const db = getDb(env);
  const video = await db
    .prepare('SELECT id FROM videos WHERE id = ? OR slug = ? LIMIT 1')
    .bind(videoIdParam, videoIdParam)
    .first();
  if (!video) return errorResponse('Video not found', 404, corsHeaders);
  const resolvedVideoId = String(video.id);
  ...
  const license = await db
    ...
    LIMIT 1
  `)
    .bind(claims.licenseId, claims.userId, resolvedVideoId, claims.deviceId)
    .first();

  if (!license) {
    return errorResponse(
      'Download license invalid or expired',
      403,
      corsHeaders,
      'license_invalid',
    );
  ...
  if (!licensedPaths?.has(normalizedAsset)) {
    return errorResponse(
      'Asset not covered by download license',
      403,
      corsHeaders,
      'asset_not_licensed',
    );
...
export async function handleRenewDownloadLicenses(
  request: Request,
  env: any,
  corsHeaders: Record<string, string>,
) {
  ...
  for (const licenseId of licenseIds) {
    const licenseRow = await db
      ...
      LIMIT 1
    `)
      .bind(licenseId, device.user_id, device.id)
      .first();
    ...
    const updated = await db
      .prepare('SELECT * FROM offline_download_licenses WHERE id = ? LIMIT 1')
      .bind(licenseId)
      .first();
    const license = await buildSignedLicense({
      licenseRow: updated,
