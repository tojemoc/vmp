...
> {
  ...
  if (targetUser.id === sourceUserId) {
    return {
      ok: false,
      code: 'same_user',
      error: 'Cannot transfer subscription to the same account',
      status: 400,
    };
  }
  ...
  try {
    ...
    console.error(
      'executeSubscriptionTransfer batch:',
...
export async function handleAccountTransferSubscription(request: any, env: any, corsHeaders: any) {
  ...
  if (!targetEmail.trim()) {
    return jsonResponse(
      { error: 'targetEmail is required', code: 'invalid_email' },
      400,
      corsHeaders,
    );
  }
