...
async function verifyPipelineWebhook(
  rawBody: string,
  sigHeader: string,
  tsHeader: string,
  secret: string,
...
export async function handleVideoPipelineStatus(
  request: Request,
  env: PipelineEnv,
  ...
  if (!secret) {
    return jsonResponse(
      { error: 'Pipeline webhook secret is not configured', code: 'secret_not_configured' },
      503,
      corsHeaders,
    );
  }
  ...
  if (!valid) {
    log({
      service: 'pipeline',
      event: 'pipeline_webhook_invalid_signature',
      level: 'warn',
      video_id: videoId,
    });
    return jsonResponse(
