...
-- Upgrade edited pages that mention VOD but still lack the Continue watching / erasure note.
UPDATE cms_pages
SET content = REPLACE(
  content,
  'Anonymous visitors do not get server-side resume.',
  'Anonymous visitors do not get server-side resume. You can remove a saved position for any video from Continue watching on your account page. To request erasure of your account and related personal data — including all saved playback positions — email the support address shown at the bottom of this page.'
),
updated_at = CURRENT_TIMESTAMP
WHERE id = 'cms-page-personal-data'
  AND instr(content, 'Continue watching on your account page') = 0
  AND instr(content, 'To request erasure of your account') = 0
  AND instr(content, 'If you request account deletion') = 0
  AND instr(content, 'on-demand video (VOD)') > 0
...
-- Edited/restructured pages with VOD but reworded body: append erasure note when still missing.
-- POSTGRES: UPDATE cms_pages SET content = (content::jsonb || '[{"type":"rich_text","content":{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"You can remove a saved position for any video from Continue watching on your account page. To request erasure of your account and related personal data — including all saved playback positions — email the support address shown at the bottom of this page."}]}]}}]'::jsonb)::text, updated_at = CURRENT_TIMESTAMP WHERE id = 'cms-page-personal-data' AND position('Continue watching on your account page' in content) = 0 AND position('To request erasure of your account' in content) = 0 AND position('If you request account deletion' in content) = 0 AND position('on-demand video (VOD)' in content) > 0 AND jsonb_typeof(content::jsonb) = 'array';
UPDATE cms_pages
SET content = json_insert(
  content,
  '$[#]',
  json('{"type":"rich_text","content":{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"You can remove a saved position for any video from Continue watching on your account page. To request erasure of your account and related personal data — including all saved playback positions — email the support address shown at the bottom of this page."}]}]}}')
),
updated_at = CURRENT_TIMESTAMP
WHERE id = 'cms-page-personal-data'
...
-- Require Continue watching + erasure/deletion markers absent so a partial disclosure that
-- already mentions Continue watching or an older deletion note is not duplicated.
-- POSTGRES: UPDATE cms_pages SET content = (content::jsonb || '[{"type":"rich_text","content":{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"If you are signed in, we also store your last playback position per on-demand video (VOD) on our servers so we can resume where you left off. Positions are updated occasionally while you watch and when you leave the page — not on every scrub of the timeline. Anonymous visitors do not get server-side resume. You can remove a saved position for any video from Continue watching on your account page. To request erasure of your account and related personal data — including all saved playback positions — email the support address shown at the bottom of this page."}]}]}}]'::jsonb)::text, updated_at = CURRENT_TIMESTAMP WHERE id = 'cms-page-personal-data' AND position('on-demand video (VOD)' in content) = 0 AND position('Continue watching on your account page' in content) = 0 AND position('To request erasure of your account' in content) = 0 AND position('If you request account deletion' in content) = 0 AND jsonb_typeof(content::jsonb) = 'array';
UPDATE cms_pages
SET content = json_insert(
  content,
  '$[#]',
