...
async function verifyAsset(
  ...
  if (hotMeta.size !== coldMeta.size) {
    throw new Error(
      `integrity check failed: size mismatch hot=${hotMeta.size} cold=${coldMeta.size} key=${hotKey}`,
    );
  }
  ...
  if (integrityMode === 'sha256') {
    ...
    try {
      ...
      await Promise.all([hotStorage.cleanupTemp(hotTmp), coldStorage.cleanupTemp(coldTmp)]);
    }
  }
}
...
export class TierOffloader {
  ...
  async demoteVideo(
    videoId: string,
    integrityMode: IntegrityMode = 'size',
    signal?: AbortSignal,
  ): Promise<void> {
    ...
    process.stdout.write(
      `${new Date().toISOString()} [offloading] demote start video=${videoId} objects=${objects.length}\n`,
    );
    for (const object of objects) {
      if (signal?.aborted) return;
      process.stdout.write(
        `${new Date().toISOString()} [offloading] demote copy key=${object.key}\n`,
      );
    ...
    process.stdout.write(
      `${new Date().toISOString()} [offloading] demote complete video=${videoId}\n`,
    );
  }
  ...
  async promoteVideo(
    ...
    if (objects.length === 0) return;
    process.stdout.write(
      `${new Date().toISOString()} [offloading] promote start video=${videoId} objects=${objects.length}\n`,
    );
    for (const object of objects) {
      if (signal?.aborted) return;
      process.stdout.write(
        `${new Date().toISOString()} [offloading] promote copy key=${object.key}\n`,
      );
    ...
    process.stdout.write(
      `${new Date().toISOString()} [offloading] promote complete video=${videoId}\n`,
    );
  }
}
