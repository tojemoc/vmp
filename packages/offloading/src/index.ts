...
async function main(): Promise<void> {
  ...
  log(
    `config r2Root=${config.r2Root} garageRoot=${config.garageRoot} listPrefix=${config.listPrefix} maxHotAgeSeconds=${config.maxHotAgeSeconds} deleteHotAfterOffload=${config.deleteHotAfterOffload}`,
  );
  ...
  throw new Error(
    'Unknown mode. Use one of: offload | demote | demote-legacy | promote | trigger-promote',
  );
}
