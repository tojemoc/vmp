...
export type HomepageRenderBlock =
  ...
  | HomepageRenderCategoryPairBlock
...
export function isPageBannerRenderBlock(
  block: HomepageRenderBlock,
): block is HomepageRenderPageBannerBlock {
...
export function ensureBlockGridPositions(blocks: HomepageLayoutBlock[]): HomepageLayoutBlock[] {
  ...
  const needsAssign = blocks.some(
    (block) => !Number.isFinite(Number(block.gridRow)) || !Number.isFinite(Number(block.gridCol)),
  );
  return needsAssign
    ? assignGridPositions(blocks)
    : blocks.map((block) => ({
        ...block,
...
export function rowPartnerBlock(
  blocks: HomepageLayoutBlock[],
  block: HomepageLayoutBlock,
): HomepageLayoutBlock | null {
  const positioned = ensureBlockGridPositions(blocks);
  ...
  return (
    positioned.find(
      (candidate) => Number(candidate.gridRow) === row && Number(candidate.gridCol) === partnerCol,
    ) ?? null
  );
}

export function blockWouldExpandToFullWidth(
  blocks: HomepageLayoutBlock[],
  block: HomepageLayoutBlock,
): boolean {
  ...
  if (partner.type === 'category' && (!partner.categoryId || partner.categoryId.trim() === ''))
    return true;
  return false;
}
...
export function orderLayoutBlocksForViewport(
  ...
  return blocks
    ...
    .sort((a, b) => {
      const ao = Number.isFinite(Number(a.block.mobileOrder))
        ? Number(a.block.mobileOrder)
        : a.index;
      const bo = Number.isFinite(Number(b.block.mobileOrder))
        ? Number(b.block.mobileOrder)
        : b.index;
      return ao - bo;
    })
    .map(({ block }) => block);
...
function buildPageBannerBlock(block: HomepageLayoutBlock): HomepageRenderPageBannerBlock | null {
  ...
  const mobileImageId =
    typeof block.mobileImageId === 'string' && block.mobileImageId.trim()
...
function layoutIncludesTopVideoBlock(blocks: HomepageLayoutBlock[]): boolean {
  for (const block of blocks) {
    ...
    if (
      ...
      block.childBlocks.some((child) => child?.type === 'top_video')
    ) {
      return true;
    }
  }
...
export function buildHomepageRenderModel({
  ...
  const sortedByNewest = [...videoById.values()].sort((a: any, b: any) =>
    compareVideosNewestFirst(a, b),
  );
  ...
  const categorySections = (placement?.categoryBlocks ?? [])
    .map((block) => {
      ...
      const variant =
        block.category?.homepage_layout_variant === 'side_mini' ? 'side_mini' : 'three_by_one';
  ...
  const sectionByCategoryId = new Map(
    categorySections.map((section) => [section.category.id, section]),
  );

  const buildLeafBlock = (
    block: HomepageLayoutChildBlock | HomepageLayoutBlock,
    id: string,
  ): HomepageRenderLeafBlock | null => {
    const type = block?.type;
    if (type !== 'featured_row' && type !== 'category' && type !== 'top_video') return null;
    const title = typeof block?.title === 'string' ? block.title : '';
    const body = typeof block?.body === 'string' ? block.body : '';
    const categoryId =
      typeof block?.categoryId === 'string' && block.categoryId.trim() ? block.categoryId : null;
  ...
  for (const row of [...rows.keys()].sort((a, b) => a - b)) {
    ...
    if (
      ...
      leftLeaf &&
      rightLeaf &&
      leftLeaf.type === 'category' &&
      rightLeaf.type === 'category'
    ) {
      const pairedPrimary: HomepageRenderLeafBlock =
        leftLeaf.type === 'category' && leftLeaf.categorySection
          ? {
              ...leftLeaf,
              categorySection: {
          ...
          : leftLeaf;
      const sideMini: HomepageRenderLeafBlock =
        rightLeaf.type === 'category' && rightLeaf.categorySection
          ? {
              ...rightLeaf,
              categorySection: {
    ...
    if (leftVisible && leftLeaf) {
      blockItems.push({
        ...leftLeaf,
        expandedFromHalf: leftConfig?.width === 'half' && !rightVisible,
      });
    }
    if (rightVisible && rightLeaf) {
      blockItems.push({
        ...rightLeaf,
        expandedFromHalf: rightConfig?.width === 'half' && !leftVisible,
      });
    }
  }

  const hasFeaturedRowBlock = blockItems.some(
    (item: any) =>
