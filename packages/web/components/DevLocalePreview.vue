<template>
  <div
    class="fixed bottom-3 left-3 z-[9999] flex flex-col gap-2 max-w-xs rounded-lg border border-amber-500/50 bg-amber-50 dark:bg-amber-950/95 text-amber-950 dark:text-amber-50 shadow-lg backdrop-blur px-3 py-2 text-xs"
    role="region"
    aria-label="Locale preview (development only)"
  >
    <p class="font-semibold leading-snug text-amber-950 dark:text-amber-50">
      Locale preview
      <span v-if="isDevLocalePreview" class="text-amber-700 dark:text-amber-300">(override)</span>
    </p>
    <p class="text-amber-800 dark:text-amber-200/90 leading-relaxed">
      Review SK/CZ copy in real UI flows. Edit
      <code class="text-amber-900 dark:text-amber-100">locales/{{ locale }}/</code>
      — HMR updates strings.
    </p>
