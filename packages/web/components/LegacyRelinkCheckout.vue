<template>
  <div class="space-y-4">
    ...
    <div v-else class="grid gap-2" :class="planGridClass">
      ...
      >
        <p
          class="text-[10px] uppercase tracking-wide mb-0.5 leading-tight text-gray-500 dark:text-gray-400"
        >
          {{ planLabel(plan) }}
        </p>
...
<script setup lang="ts">
  ...
  const visiblePlans = computed(() => allowedPlans.value.filter((plan) => planPrice(plan) != null));
  ...
  async function loadPrices() {
    ...
    try {
      ...
      prices.value = {
        monthly:
          legacyRaw.monthly != null
            ? parseNullablePrice(legacyRaw.monthly)
            : parseNullablePrice(data.monthly),
        yearly:
          legacyRaw.yearly != null
            ? parseNullablePrice(legacyRaw.yearly)
            : parseNullablePrice(data.yearly),
        club:
          legacyRaw.club != null
            ? parseNullablePrice(legacyRaw.club)
            : parseNullablePrice(data.club),
      };
