<template>
  ...
  >
    <div
      class="w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-xl overflow-hidden"
    >
      <div
        class="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-start justify-between gap-3"
      >
        <div>
          <h2 id="pwa-login-title" class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ strings.pwaLoginTitle }}
          </h2>
          <p class="text-gray-600 dark:text-gray-400 text-sm mt-1 leading-relaxed">
            {{ strings.pwaLoginIntro }}
          </p>
        </div>
        <button
          type="button"
          class="text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 text-sm shrink-0"
      ...
      <div class="px-5 py-3 text-xs text-gray-500 dark:text-gray-500">
      ...
      <div v-if="step === 1" class="px-5 pb-5 space-y-4">
        <label
          ...
          >{{ strings.pwaLoginEmailLabel }}</label
        >
        <input
          id="pwa-login-email"
          v-model="email"
          type="email"
          autocomplete="email"
          class="w-full px-3 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white text-sm"
          :disabled="loading"
        ...
        <button
          type="button"
          class="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white dark:text-white font-semibold rounded-lg text-sm"
      ...
      <div v-else-if="step === 2" class="px-5 pb-5 space-y-4">
        <p
          ...
          class="text-sm text-emerald-700 dark:text-emerald-300/90 leading-relaxed"
        >
          {{ strings.pwaLoginPushAlreadyGranted }}
        </p>
        <p v-else class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          {{ strings.pwaLoginPushStep }}
        </p>
        <p v-if="pushDenied" class="text-sm text-amber-700 dark:text-amber-300/90 leading-relaxed">
          {{ strings.pwaLoginPushDenied }}
        </p>
        <p v-if="errorMessage" class="text-xs text-red-600 dark:text-red-400">{{ errorMessage }}</p>
        <button
          v-if="!pushDenied"
        ...
        >
          {{
            loading
              ? strings.pwaLoginWorking
              : pushAlreadyGranted
                ? strings.pwaLoginResendEmail
                : strings.pwaLoginAllowNotifications
          }}
        </button>
        <button
          v-if="pushAlreadyGranted"
          type="button"
          class="w-full py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 bg-white dark:bg-transparent rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-800"
          :disabled="loading"
      ...
      <div v-else class="px-5 pb-5 space-y-4">
        <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          {{ strings.pwaLoginCheckEmail }}
        </p>
        <p class="text-xs text-gray-500 dark:text-gray-500">{{ strings.pwaLoginCheckEmailHint }}</p>
        <button
          type="button"
          class="w-full py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 bg-white dark:bg-transparent rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-800"
          @click="emit('dismiss')"
...
<script setup lang="ts">
  ...
  async function turnOffPushPermission() {
    ...
    try {
      ...
      try {
        localStorage.removeItem(PWA_LOGIN_EMAIL_KEY);
      } catch {
        /* ignore */
      }
    } catch (e: unknown) {
  ...
  async function goStep2() {
    ...
    try {
      ...
      try {
        localStorage.setItem(PWA_LOGIN_EMAIL_KEY, normalized);
      } catch {
        /* ignore */
      }
  ...
  async function requestPushAndSubscribe() {
    ...
    try {
      ...
      if (!subJson.endpoint || !p256dh || !auth) {
        throw new Error(strings.notificationsEnableFailed);
      }
    ...
    async (isOpen) => {
      if (isOpen) {
        ...
        if (email.value) {
          await detectPushAlreadyGranted();
          const permGranted =
            import.meta.client && 'Notification' in window && Notification.permission === 'granted';
