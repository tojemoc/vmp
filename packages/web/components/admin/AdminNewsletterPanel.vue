  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
        Compose campaigns, manage drafts, and view sent campaigns.
      </p>
    </div>
    ...
    <div v-if="activeSubTab === 'compose'" class="grid gap-6 lg:grid-cols-2">
      <div class="space-y-4">
        ...
        <label class="block text-sm font-medium text-gray-900 dark:text-white">
          ...
          <textarea
            v-model="body"
            rows="14"
            class="mt-1 w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-mono text-sm min-h-[12rem]"
          />
        </label>
        ...
        <div class="flex flex-wrap gap-2">
          <button
            type="button"
            class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50"
          ...
          <div class="relative">
            ...
            <div
              ...
              class="absolute z-10 mt-2 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg space-y-2 min-w-[16rem]"
            >
              <input
      ...
      <div>
        ...
        <div
          v-else
          class="rounded-lg border border-dashed border-gray-300 dark:border-gray-600 min-h-[24rem] flex items-center justify-center text-sm text-gray-400"
        >
    ...
    <div v-else-if="activeSubTab === 'drafts'" class="space-y-3">
      ...
      <div v-if="!drafts.length" class="text-sm text-gray-500 dark:text-gray-400">
        No drafts yet.
      </div>
      <div
      ...
      >
        <div>
          ...
          <span
            v-if="draft.scheduledAt"
            class="inline-flex mt-1 px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200"
            >Scheduled</span
          >
        </div>
        <div class="flex gap-2 text-sm">
          <button
            ...
            @click="loadDraftIntoCompose(draft)"
          >
            Edit
          </button>
          <button
    ...
    <div v-else class="space-y-3">
      <p class="text-xs text-gray-500 dark:text-gray-400">
        <span v-if="lastCampaignsOkAt"
          >Last refreshed: {{ new Date(lastCampaignsOkAt).toLocaleString() }}</span
        >
        <span v-if="lastCampaignsError" class="text-red-600 dark:text-red-400">
          · {{ lastCampaignsError }}</span
        >
      </p>
...
<script setup lang="ts">
  ...
  async function loadDrafts() {
    const res = await fetch(`${config.public.apiUrl}/api/admin/newsletter/drafts`, {
      headers: authHeader(),
    });
    if (!res.ok) return;
    const data = await res.json();
