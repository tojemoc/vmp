<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <p v-if="mobileOrderMode" class="text-xs text-gray-500 dark:text-gray-400">
          Reorder blocks for viewports under 1024px. Hidden blocks are dimmed.
        </p>
      </div>
      <button
        type="button"
        class="inline-flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white dark:text-white text-sm font-semibold rounded-lg"
        @click="addBlock('top_video')"
      >
    ...
    <p v-if="!mobileOrderMode" class="text-xs text-gray-500 dark:text-gray-400">
      Use <strong class="font-medium">Half width</strong> for side-by-side rows. Two half blocks on
      the same row pair automatically — left is the main category (2×2), right is the side-mini
      column.
    </p>
      ...
      <template v-for="slot in editorSlots" :key="slot.key">
        ...
        >
          <div class="flex items-center gap-3 mb-3 flex-wrap">
            <span
              v-if="!mobileOrderMode"
              class="cursor-move text-gray-500 dark:text-gray-400"
            ...
            <span
              ...
              class="cursor-move text-gray-500 dark:text-gray-400"
              :draggable="true"
              @dragstart.stop="onDragStart(slot.block.id)"
              @dragover.prevent
              @drop.stop="onDrop(slot.block.id)"
              >☰</span
            >
            <span class="text-xs font-mono text-gray-500 dark:text-gray-400"
              >{{ positionLabel(slot.block) }}</span
            >
            <select
            ...
            >
              <option v-for="t in blockTypeOptions(slot.block.type)" :key="t" :value="t">
                {{ t }}
              </option>
            </select>
            <select
            ...
            <button
              ...
              @click="removeBlock(slot.block.id)"
            >
              Remove
            </button>
          </div>
          ...
          <label
            v-if="mobileOrderMode"
            class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 mb-3"
          >
            <input
          ...
          <div class="grid gap-3">
            ...
            >
              ...
              <option
                ...
                :key="`block-cat-${slot.block.id}-${cat.id}`"
                :value="cat.id"
              >
                {{ cat.name }}
              </option>
            ...
            <div
              v-if="slot.block.type === 'page_banner'"
            ...
            >
              ...
              >
                ...
                <option
                  ...
                  :key="`banner-page-${slot.block.id}-${page.id}`"
                  :value="page.slug"
                >
                  {{ page.title }}
                  (/{{ page.slug }})
            ...
            <div
              v-if="(slot.block.type === 'split_horizontal' || slot.block.type === 'split_vertical') && slot.block.childBlocks"
              class="grid gap-3 rounded-md border border-dashed border-gray-300 dark:border-gray-700 p-3"
            >
              <div
                v-for="(child, childIndex) in slot.block.childBlocks"
                :key="`${slot.block.id}-child-${childIndex}`"
              ...
              >
                <div class="text-xs font-semibold text-gray-600 dark:text-gray-400">
                  Child block {{ childIndex + 1 }}
                </div>
                <select
                  v-model="child.type"
                  class="px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white"
                >
                  <option v-for="t in leafBlockTypeOptions(child.type)" :key="t" :value="t">
                    {{ t }}
                  </option>
                </select>
                <input
                  v-model="child.title"
                  type="text"
                  placeholder="Child block title"
                  class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
                >
                <textarea
                  v-model="child.body"
                  rows="2"
                  placeholder="Child block copy"
                  class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
                />
                <select
                ...
                >
                  ...
                  <option
                    v-for="cat in categories"
                    :key="`child-cat-${slot.block.id}-${childIndex}-${cat.id}`"
                    :value="cat.id"
                  >
                    {{ cat.name }}
                  </option>
...
<script setup lang="ts">
  ...
  function reorderBlocks(fromId: string, targetId: string, insertAfterTarget = false) {
    ...
    const reordered = ordered.map(
      (block) => blocks.value.find((entry) => entry.id === block.id) ?? block,
    );
  ...
  function onDropPartnerSlot(anchorBlockId: string) {
    ...
    if (
      moved &&
      moved.type !== 'split_horizontal' &&
      moved.type !== 'split_vertical' &&
      moved.type !== 'page_banner'
    ) {
      moved.width = 'half';
    }
    blocks.value = assignGridPositions(reordered);
