...
import { clearStoredPwaHandoffCode, readStoredPwaHandoffCode } from '~/utils/pwa-auth-idb';

export default defineNuxtPlugin(() => {
  const route = useRoute();
  const router = useRouter();
  const { redeemPwaHandoff, isLoggedIn, initialised, refreshSession, ensureFreshSession } =
    useAuth();
  ...
  async function tryRedeemFromUrlOrIdb() {
    ...
    const fromQuery =
      typeof route.query.pwa_auth_handoff === 'string' ? route.query.pwa_auth_handoff.trim() : '';
    if (fromQuery) {
