...
describe('isChunkLoadErrorReason', () => {
  ...
  it('matches Chromium dynamic import fetch failures', () => {
    assert.equal(
      isChunkLoadErrorReason(
        new TypeError('Failed to fetch dynamically imported module: /_nuxt/watch.abc.js'),
      ),
      true,
    );
  });
  ...
  it('does not match unrelated promise rejections', () => {
    assert.equal(
      isChunkLoadErrorReason(new TypeError('Cannot read properties of undefined')),
      false,
    );
  });
  ...
  it('matches transient 503 asset failures', () => {
    assert.equal(
      isChunkLoadErrorReason(
        new TypeError('Failed to fetch /_nuxt/app.abc.js: 503 Service Unavailable'),
      ),
      true,
    );
  });
...
describe('shouldAttemptChunkReload', () => {
  ...
  it('throttles reload attempts when storage operations throw or storage is unavailable', () => {
    ...
    assert.equal(
      shouldAttemptChunkReload(blockedStorage, now + CHUNK_RELOAD_THROTTLE_MS - 1),
      false,
    );
