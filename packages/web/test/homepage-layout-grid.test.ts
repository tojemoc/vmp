...
describe('assignGridPositions', () => {
  it('places full-width blocks on their own row', () => {
    ...
    assert.deepEqual(
      blocks.map((block) => [block.id, block.gridRow, block.gridCol, block.width]),
      [
        ['a', 0, 0, 'full'],
        ['b', 1, 0, 'half'],
        ['c', 1, 1, 'half'],
...
describe('buildHomepageRenderModel grid rows', () => {
    ...
    categoryBlocks: [
      {
        category: {
          ...
          slug: 'main',
          name: 'Main',
          homepage_layout_variant: 'three_by_one',
        },
      ...
      },
      {
        category: {
          ...
          slug: 'side',
          name: 'Side',
          homepage_layout_variant: 'side_mini',
        },
        visible: [{ id: 'v3' }, { id: 'v4' }],
        overflow: [],
      },
  ...
  it('hides empty categories and expands the visible half block', () => {
    ...
    assert.equal(
      model.blockItems.some((item: any) => item.categoryId === 'cat-empty'),
      false,
    );
  });
  ...
  it('suppresses featured videos from categories only when a featured_row block exists', () => {
    ...
    const featured = withFeaturedRow.blockItems.find(
      (item: any) => item.type === 'featured_row',
    ) as any;
    const category = withFeaturedRow.blockItems.find(
      (item: any) => item.type === 'category',
    ) as any;
    assert.equal(featured?.videos?.[0]?.id, 'v-pin');
    assert.equal(
      category?.categorySection?.visible.some((video: any) => video.id === 'v-pin'),
      false,
    );
    ...
    const loneCategory = withoutFeaturedRow.blockItems.find(
      (item: any) => item.type === 'category',
    ) as any;
