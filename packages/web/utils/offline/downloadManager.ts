...
import { buildOfflineMasterPlaylist, rewritePlaylistForOffline } from './localManifest';
import {
  deleteOfflineVideo,
...
import type { DownloadProgress, RenewLicenseResult, StoredDownload } from './types';
...
export async function revalidateOfflineLicenses(apiUrl: string): Promise<RenewLicenseResult[]> {
  ...
  for (const record of downloads) {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/api/downloads/${encodeURIComponent(record.videoId)}/authorize`,
        {
          method: 'POST',
          credentials: 'include',
          headers: {
      ...
      if (
        data.license?.manifestVersion > record.manifestVersion ||
...
export async function getStorageSummary(): Promise<{
  ...
  return {
    ...
    downloadCount: downloads.filter(
      (d) => d.status === 'completed' || d.status === 'update_available',
    ).length,
  };
}
