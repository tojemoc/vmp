...
export async function listQueuedDownloads(): Promise<string[]> {
  ...
  try {
    const db = await openOfflineIdb();
    const rows = await new Promise<Array<{ videoId: string; queuedAt: string }>>(
      (resolve, reject) => {
        const tx = db.transaction(OFFLINE_STORE_QUEUE, 'readonly');
        tx.onerror = () => reject(tx.error);
        const req = tx.objectStore(OFFLINE_STORE_QUEUE).getAll();
        req.onsuccess = () =>
          resolve((req.result as Array<{ videoId: string; queuedAt: string }>) ?? []);
        req.onerror = () => reject(req.error);
      },
    );
    return rows.sort((a, b) => a.queuedAt.localeCompare(b.queuedAt)).map((r) => r.videoId);
  } catch {
    return [];
  }
}
