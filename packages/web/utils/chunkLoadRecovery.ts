...
export function isCriticalAssetLoadTarget(
  target: EventTarget | null | undefined,
): target is HTMLScriptElement | HTMLLinkElement {
  if (!target || typeof target !== 'object') return false;
...
function errorMessages(reason: unknown): string[] {
  ...
  if (reason && typeof reason === 'object') {
    ...
    return [record.name, record.message, record.type].filter(
...
export function shouldAttemptChunkReload(
  ...
  if (
    fallbackChunkReloadAttemptedAt !== null &&
    now - fallbackChunkReloadAttemptedAt < CHUNK_RELOAD_THROTTLE_MS
  ) {
    return false;
  }
