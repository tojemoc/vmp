      ...
      <div v-else-if="videos.length > 0" class="space-y-8">
        ...
        <template v-for="block in renderedBlocks" :key="`extra-${block.id}`">
          <section v-if="block.type === 'cta'" class="p-6 rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-950/40 dark:border-blue-800">
            ...
            <p class="text-gray-800 dark:text-gray-200">{{ block.body || strings.ctaBodyDefault }}</p>
          </section>
          ...
          <section v-else-if="block.type === 'text_split'" class="grid md:grid-cols-2 gap-4">
            <div class="p-4 rounded-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
              <h3 class="font-semibold text-gray-900 dark:text-white mb-2">{{ block.title || strings.textSplitTitleDefault }}</h3>
              <p class="text-gray-600 dark:text-gray-400">{{ block.body || strings.textSplitBodyDefault }}</p>
            </div>
            <div class="p-4 rounded-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
              <h3 class="font-semibold text-gray-900 dark:text-white mb-2">{{ strings.textSplitCompareTitle }}</h3>
              <p class="text-gray-600 dark:text-gray-400">{{ strings.textSplitCompareBody }}</p>
            </div>
          </section>
        </template>
      </div>
...
watchEffect(() => {
  ...
  homepageHeroTitle.value = content?.title || strings.heroTitleDefault
  homepageHeroSubtitle.value = content?.subtitle || strings.heroSubtitleDefault
