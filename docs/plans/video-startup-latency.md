...
- [x] Aggressive async above-the-fold HLS startup prefetch (init + first **3** segments on the cheapest rung + audio): eager queue of ~12 cards (logged-in and anonymous; anon free-preview limits count only intentional `/watch` opens via `X-VMP-Watch-View`, not prefetch), plus IntersectionObserver with **one viewport** of `rootMargin` so content just below the fold warms too. Category grids use the same queue.
