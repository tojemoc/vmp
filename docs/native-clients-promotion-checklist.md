...
| S5 | **Universal Links / App Links** | Set `MOBILE_ANDROID_SHA256_CERT_FINGERPRINTS` + `MOBILE_APPLE_APP_IDS` repo vars so `/.well-known/assetlinks.json` and `/.well-known/apple-app-site-association` return 200 on production `FRONTEND_HOST` (routes ship in the web Worker; unset ⇒ 404); verified with Apple/Google tools. Until S5 is live, Android `/auth/verify` uses package-targeted `intent://` handoff into the APK (see `packages/web/utils/nativeAppHandoff.ts`) so PoC sideloads still work. **iOS SideStore:** Universal Links need AASA with a stable Team ID (not available while each tester re-signs); do **not** put raw magic-link tokens in `vmp://` URLs — install-bound handoff codes are deferred (see plan). **Owner:** repository maintainer (Cloudflare web Worker host) | Maintainer |
| S6 | **`vmp://` not primary** | Env gates stay fail-closed: custom scheme is active only when `EXPO_PUBLIC_ENABLE_VMP_SCHEME=1`/`true` **and** `EXPO_PUBLIC_DISABLE_VMP_SCHEME` is not `1`/`true` (`DISABLE` always wins). **SideStore / PoC artifact builds** (`development` / `beta` / `nightly` via Mobile artifacts) may enable the scheme for local deep-link experiments. **App Store / TestFlight (`flavor=release`)** must keep it disabled — workflow forces `ENABLE=0` / `DISABLE=1` even if the input is true; HTTPS Universal/App Links only | Maintainer |
...
| When does `vmp://` become dev-only? | **Default off** in app code (fail-closed env gates). **Exception:** SideStore/PoC Mobile artifacts (`development`/`beta`/`nightly`) may set `EXPO_PUBLIC_ENABLE_VMP_SCHEME=1`. **`flavor=release`** always forces the scheme off. `DISABLE=1` always wins. Store / TestFlight builds must omit ENABLE (**S6**). |
