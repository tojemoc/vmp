...
| S6 | **`vmp://` not primary** | Env gates stay fail-closed: custom scheme is active only when `EXPO_PUBLIC_ENABLE_VMP_SCHEME=1`/`true` **and** `EXPO_PUBLIC_DISABLE_VMP_SCHEME` is not `1`/`true` (`DISABLE` always wins). **Mobile artifact / SideStore / release CI builds always force the scheme off** (no install-bound handoff yet). Opt-in is for **local developer machines only**. Web never embeds magic-link tokens or unbound handoff codes in `vmp://`. HTTPS Universal/App Links only for distributed clients (**S5**) | Maintainer |
...
| When does `vmp://` become dev-only? | **Always off** for Mobile artifact / SideStore / release CI builds. App code stays fail-closed. Opt-in (`EXPO_PUBLIC_ENABLE_VMP_SCHEME=1`) is for **local developer machines only** until install-bound handoff exists (**S6**). |
