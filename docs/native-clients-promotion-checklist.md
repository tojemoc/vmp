...
| S6 | **`vmp://` not primary** | Custom scheme is dev-only fallback; store build relies on verified HTTPS deep links | Maintainer |
...
| S8 | **TV pairing labels** | Unverified `deviceName` / `devicePlatform` accepted with warning, or attestation added | Maintainer |
...
| S10 | **Pairing preview abuse** | Preview endpoint rate-limited per code (not only per IP) if exposed to production traffic | Maintainer |

## Phase 0 pairing contract

Not frozen on merge of this PoC. **Additive** changes (new optional fields, new endpoints) are allowed through the Tier 2 TV PoC. **Breaking** changes (rename, remove, change status codes/shapes) require a maintainer waiver in the PR that ships them. Stability is declared when the Tier 2 PoC ships, not at Phase 0 merge.

## TV re-pair UX (lost `ready` poll)

**Accepted for Tier 2 launch:** if the TV loses the HTTP response after the server marks the code redeemed, the user starts a new pairing flow (new code + phone re-approval). An idempotency window is a later ADR, not the default.
...
| Re-pair UX accepted? | **Yes for Tier 2 launch** — new code + phone re-approval. Idempotency window is a later ADR. |
