...
| S5 | **Universal Links / App Links** | Set `MOBILE_ANDROID_SHA256_CERT_FINGERPRINTS` + `MOBILE_APPLE_APP_IDS` repo vars so `/.well-known/assetlinks.json` and `/.well-known/apple-app-site-association` return 200 on production `FRONTEND_HOST` (routes ship in the web Worker; unset ⇒ 404); verified with Apple/Google tools. Until S5 is live, Android `/auth/verify` uses package-targeted `intent://` handoff into the APK (see `packages/web/utils/nativeAppHandoff.ts`) so PoC sideloads still work. **iOS SideStore:** Universal Links need AASA with a stable Team ID (not available while each tester re-signs). **Temporary staging-only escape hatch:** `flavor=development` + `enable_custom_scheme` IPA, plus staging web double-confirm + D1 `insecure_native_scheme_acks` before `vmp://` (see plan / S6). Install-bound handoff codes remain the long-term fix. **Owner:** repository maintainer (Cloudflare web Worker host) | Maintainer |
| S6 | **`vmp://` not primary** | Env gates stay fail-closed: custom scheme is active only when `EXPO_PUBLIC_ENABLE_VMP_SCHEME=1`/`true` **and** `EXPO_PUBLIC_DISABLE_VMP_SCHEME` is not `1`/`true` (`DISABLE` always wins). **release/beta/nightly** Mobile artifact builds always force the scheme off. **`flavor=development` + `enable_custom_scheme`** may enable the scheme for SideStore PoC. Staging web may open `vmp://` with a magic-link token **only** after two explicit confirms and a D1 acknowledgment, and only when `ALLOW_INSECURE_NATIVE_VMP_SCHEME=1` (staging deploy). Production / beta web never enable that path. HTTPS Universal/App Links remain required for distributed clients (**S5**) | Maintainer |
...
| When does `vmp://` become dev-only? | **Always off** for release/beta/nightly Mobile artifact builds. Optional for `flavor=development` + `enable_custom_scheme` (SideStore), gated on staging by double-confirm + D1 ack (`ALLOW_INSECURE_NATIVE_VMP_SCHEME`). Local opt-in still works. Long-term: Universal Links (**S5**) or install-bound handoff (**S6**). |
