...
| S5 | **Universal Links / App Links** | Set `MOBILE_ANDROID_SHA256_CERT_FINGERPRINTS` + `MOBILE_APPLE_APP_IDS` repo vars so `/.well-known/assetlinks.json` and `/.well-known/apple-app-site-association` return 200 on production `FRONTEND_HOST` (routes ship in the web Worker; unset ⇒ 404); verified with Apple/Google tools. Until S5 is live, `/auth/verify` hands off into the native client without consuming the token first: Android via package-targeted `intent://`, iOS SideStore/PoC via `vmp://auth/verify` (see `packages/web/utils/nativeAppHandoff.ts`). **SideStore note:** each tester re-signs with their own Apple Team ID, so a single AASA entry cannot cover PoC IPAs — keep `enable_custom_scheme` on for those builds. **Owner:** repository maintainer (Cloudflare web Worker host) | Maintainer |
