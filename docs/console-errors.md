...
3. **Wrong hostname / stale Worker** — Confirm the URL is served by `vmp-web-worker-dev` or `vmp-web-worker-prod` (not a deprecated Pages project). Primary analytics are PostHog (product) and Sentry (errors); GTM is admin opt-in.
